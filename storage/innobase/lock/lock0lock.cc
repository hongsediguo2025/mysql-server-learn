...
static void lock_table_resurrect(dict_table_t *table, trx_t *trx,
                                 lock_mode mode);
static dberr_t lock_preserve_resurrect_table_lock(dict_table_t *table,
                                                  trx_t *trx, lock_mode mode);
...
static bool lock_preserve_record_entry_has_conflict(
  ...
  for (uint32_t heap_no = 0; heap_no < entry.n_bits; ++heap_no) {
    ...
    if (lock_rec_other_has_conflicting(precise_mode, block, heap_no, trx) !=
        nullptr) {
      return true;
    }
  }
...
static dberr_t lock_preserve_import_predicate_lock(
  ...
  if (!lock_preserve_skip_record_lock_table_resurrect()) {
    const dberr_t table_err =
        lock_preserve_resurrect_table_lock(table, trx, LOCK_IS);
