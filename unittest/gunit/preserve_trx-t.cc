...
TEST(PreservedTrxPhysicalFence,
     EpochDigestCommitmentIsOrderIndependentAndRejectsDuplicateToken) {
  const Preserve_trx_epoch_physical_digest_input first{
      "token-1", 1, std::string(64, '1'), std::string(64, '2'),
      std::string(64, '3')};
  const Preserve_trx_epoch_physical_digest_input second{
      "token-2", 2, std::string(64, '4'), std::string(64, '5'),
      std::string(64, '6')};
  std::string lock_forward;
  std::string page_forward;
  std::string dictionary_forward;
  ASSERT_TRUE(preserved_trx_compute_epoch_physical_digest_commitments(
      {first, second}, &lock_forward, &page_forward, &dictionary_forward));
  std::string lock_reverse;
  std::string page_reverse;
  std::string dictionary_reverse;
  ASSERT_TRUE(preserved_trx_compute_epoch_physical_digest_commitments(
      {second, first}, &lock_reverse, &page_reverse, &dictionary_reverse));
  EXPECT_EQ(lock_forward, lock_reverse);
  EXPECT_EQ(page_forward, page_reverse);
  EXPECT_EQ(dictionary_forward, dictionary_reverse);
  EXPECT_FALSE(preserved_trx_compute_epoch_physical_digest_commitments(
...
TEST_F(PreservedTrxNativeBinlogPrepared,
       StrictResourcesMintReceiverPrepareCapabilityFromOwnKey) {
  ...
  ASSERT_EQ(Mysql_binlog_preserve_cache_status::OK,
            resources.prepare_native_binlog_handle_for_receiver(facts,
                                                                 &reader));
  EXPECT_TRUE(resources.has_native_binlog_handle());
  EXPECT_GT(reader.read_calls(), 1U);

  facts.identity.token = "wrong-token";
  facts.canonical_digest.clear();
  ASSERT_TRUE(mysql_binlog_preserve_finalize_cache_facts(&facts));
  ChunkedBinlogPayloadReader wrong_reader(payload, 4096);
  auto wrong_resources = acquire_prepared_resources(
      key, 128, native_memory_bytes,
      mysql_binlog_preserve_native_fd_count_required(facts),
      mysql_binlog_preserve_native_tmpdir_bytes_required(facts));
  EXPECT_EQ(Mysql_binlog_preserve_cache_status::CAPABILITY_REJECTED,
...
TEST_F(PreservedTrxNativeBinlogPrepared,
       ReceiverFactsPreserveBinlogSavepointCacheStates) {
  const std::string payload(64 * 1024, 'v');
  ...
  metadata.binlog_cache_size = payload.size();
  metadata.binlog_cache_event_counter = 9;
  metadata.binlog_cache_with_rbr = true;
  metadata.binlog_cache_with_start = true;
  metadata.binlog_cache_with_content = true;
  metadata.mdl_descriptors_payload.assign(4, '\0');
  ...
  auto append_u64 = [](std::string *out, uint64_t value) {
    for (uint shift = 0; shift < 64; shift += 8)
      out->push_back(static_cast<char>((value >> shift) & 0xff));
  };
  append_u32(&metadata.sql_savepoints_payload, 1);
  append_u16(&metadata.sql_savepoints_payload, 2);
  append_u16(&metadata.sql_savepoints_payload, 2);
  append_u32(&metadata.sql_savepoints_payload, 0);
  append_u32(&metadata.sql_savepoints_payload, 0);
  append_u64(&metadata.sql_savepoints_payload, 37);
  append_u32(&metadata.sql_savepoints_payload,
             (1U << 3) | (1U << 4) | (1U << 6));
  append_u64(&metadata.sql_savepoints_payload, 3);
  metadata.sql_savepoints_payload.append("s1", 2);
  ...
  Mysql_binlog_preserve_cache_facts facts;
  ASSERT_TRUE(preserved_trx_build_native_binlog_cache_facts(
      metadata, identity, descriptor, 17, 23, &facts));
  ASSERT_EQ(1U, facts.cache_states.size());
  EXPECT_EQ(37U, facts.cache_states[0].position);
  EXPECT_TRUE(facts.cache_states[0].with_rbr);
  EXPECT_TRUE(facts.cache_states[0].with_start);
  EXPECT_TRUE(facts.cache_states[0].with_content);
  EXPECT_EQ(payload.size(), facts.cache_length);
  EXPECT_EQ(descriptor.digest, facts.payload_sha256);
  EXPECT_FALSE(facts.canonical_digest.empty());
}
...
TEST(PreservedTrxRecordLockImportMetrics, AddAccumulatesIoBreakdown) {
  ...
  total.stable_page_hits = 2;
  total.image_resolves = 3;
  total.bitmap_pages = 4;
  total.bitmap_bits = 5;
  total.page_get_us = 6;
  total.page_get_count = 7;
  total.table_open_us = 8;
  total.prefetch_pages = 9;
  total.prefetch_bytes = 10;
  total.prefetch_residency_pages = 11;
  total.prefetch_resident_pages = 12;
  total.prefetch_io_pending_pages = 13;
  total.prefetch_missing_pages = 14;

  trx_preserve_record_lock_import_metrics_t delta;
  delta.record_entries = 10;
  delta.stable_page_hits = 20;
  delta.image_resolves = 30;
  delta.bitmap_pages = 40;
  delta.bitmap_bits = 50;
  delta.page_get_us = 60;
  delta.page_get_count = 70;
  delta.table_open_us = 80;
  delta.prefetch_pages = 90;
  delta.prefetch_bytes = 100;
  delta.prefetch_residency_pages = 110;
  delta.prefetch_resident_pages = 120;
  delta.prefetch_io_pending_pages = 130;
  delta.prefetch_missing_pages = 140;

  total.add(delta);

  EXPECT_EQ(11U, total.record_entries);
  EXPECT_EQ(22U, total.stable_page_hits);
  EXPECT_EQ(33U, total.image_resolves);
  EXPECT_EQ(44U, total.bitmap_pages);
  EXPECT_EQ(55U, total.bitmap_bits);
  EXPECT_EQ(66U, total.page_get_us);
  EXPECT_EQ(77U, total.page_get_count);
  EXPECT_EQ(88U, total.table_open_us);
  EXPECT_EQ(99U, total.prefetch_pages);
  EXPECT_EQ(110U, total.prefetch_bytes);
  EXPECT_EQ(121U, total.prefetch_residency_pages);
  EXPECT_EQ(132U, total.prefetch_resident_pages);
  EXPECT_EQ(143U, total.prefetch_io_pending_pages);
  EXPECT_EQ(154U, total.prefetch_missing_pages);
}
...
constexpr size_t kTestFilAddrPage = 0;
constexpr size_t kTestFilAddrByte = 4;
constexpr size_t kTestFilAddrSize = 6;
constexpr size_t kTestFlstPrev = 0;
constexpr size_t kTestFlstNext = kTestFilAddrSize;
constexpr size_t kTestFlstLen = 0;
constexpr size_t kTestFlstFirst = 4;
constexpr size_t kTestFlstLast = 4 + kTestFilAddrSize;
constexpr uint32_t kTestFilNull = std::numeric_limits<uint32_t>::max();
constexpr size_t kTestFilPageData = 38;
constexpr size_t kTestFsegHeaderSize = 10;
constexpr size_t kTestFlstNodeSize = 2 * kTestFilAddrSize;
constexpr size_t kTestTrxUndoPageHdr = kTestFilPageData;
constexpr size_t kTestTrxUndoPageNode = 6;
constexpr size_t kTestTrxUndoPageHdrSize = 6 + kTestFlstNodeSize;
constexpr size_t kTestTrxUndoSegHdr =
    kTestTrxUndoPageHdr + kTestTrxUndoPageHdrSize;
constexpr size_t kTestTrxUndoPageList = 4 + kTestFsegHeaderSize;

static void test_mach_write_to_2(unsigned char *ptr, uint16_t value) {
  ptr[0] = static_cast<unsigned char>((value >> 8) & 0xff);
  ptr[1] = static_cast<unsigned char>(value & 0xff);
}

static void test_mach_write_to_4(unsigned char *ptr, uint32_t value) {
  ptr[0] = static_cast<unsigned char>((value >> 24) & 0xff);
  ptr[1] = static_cast<unsigned char>((value >> 16) & 0xff);
  ptr[2] = static_cast<unsigned char>((value >> 8) & 0xff);
  ptr[3] = static_cast<unsigned char>(value & 0xff);
}
...
class Transfer_receiver_config_guard {
 public:
  Transfer_receiver_config_guard()
      : m_preserve_enable(preserve_trx_enable),
        ...
        m_server_uuid_ptr(server_uuid_ptr) {}
  ...
  ~Transfer_receiver_config_guard() {
    ...
    std::snprintf(server_uuid, UUID_LENGTH + 1, "%s",
                  m_server_uuid.c_str());
    server_uuid_ptr = m_server_uuid_ptr;
  }
  ...
  void allow(const char *source_uuid, const char *target_uuid) {
    ...
    std::snprintf(server_uuid, UUID_LENGTH + 1, "%s", target_uuid);
    server_uuid_ptr =
        std::strlen(target_uuid) == UUID_LENGTH ? server_uuid
                                                : m_server_uuid.c_str();
  }
 ...
 private:
  ...
  const char *m_server_uuid_ptr{nullptr};
};
...
class Transfer_credentials_guard {
 public:
  Transfer_credentials_guard() { Rpl_channel_credentials::get_instance().reset(); }

  ~Transfer_credentials_guard() {
    Rpl_channel_credentials::get_instance().reset();
  }

  void store(const char *credential_name, const char *username,
             const char *password,
             const char *auth_plugin = "mysql_native_password") {
    ASSERT_EQ(0, Rpl_channel_credentials::get_instance().store_credentials(
                     credential_name, const_cast<char *>(username),
                     const_cast<char *>(password),
                     const_cast<char *>(auth_plugin)));
  }
};
...
std::string metadata_only_record_locks_payload() {
  std::string payload;
  append_le32(&payload, 1);  // lock count
  append_le64(&payload, 1);  // table id
  append_le64(&payload, 2);  // index id
  ...
  append_le32(&payload, 4);  // page number
  append_le32(&payload, 35); // type_mode: LOCK_REC | LOCK_X
  append_le32(&payload, 80); // n_bits including native bitmap margin
  append_le64(&payload, 5);  // page lsn
  append_le32(&payload, 8);  // page n_heap
  append_le32(&payload, 4);  // stable heap identity length
  append_le32(&payload, 0);  // no record images
  append_le32(&payload, 10); // bitmap length
  append_le32(&payload, 2);  // locked heap number
  payload.push_back('\4');
  payload.append(9, '\0');
  return payload;
}
...
bool transfer_metadata_dict_lease_revalidate(void *opaque_lease,
                                             const std::string &,
                                             dict_index_t **index) {
  if (opaque_lease == nullptr || index == nullptr) return false;
  *index = reinterpret_cast<dict_index_t *>(static_cast<uintptr_t>(1));
...
void transfer_metadata_dict_lease_release(void *opaque_lease) {
...
int g_promotion_adopt_counting_success_calls = 0;
std::atomic<int> g_promotion_adopt_parallel_active{0};
std::atomic<int> g_promotion_adopt_parallel_max_active{0};
std::atomic<int> g_promotion_adopt_parallel_calls{0};

bool promotion_adopt_counting_success_for_test(
    const std::string &preserve_dir, const Preserved_trx_bundle &bundle,
    Preserve_trx_promotion_token_result *token_result) {
  ++g_promotion_adopt_counting_success_calls;
  return promotion_adopt_success_for_test(preserve_dir, bundle, token_result);
}

bool promotion_adopt_parallel_probe_for_test(
    const std::string &preserve_dir, const Preserved_trx_bundle &bundle,
    Preserve_trx_promotion_token_result *token_result) {
  ++g_promotion_adopt_parallel_calls;
  const int active = g_promotion_adopt_parallel_active.fetch_add(1) + 1;
  int observed = g_promotion_adopt_parallel_max_active.load();
  while (active > observed &&
         !g_promotion_adopt_parallel_max_active.compare_exchange_weak(
             observed, active)) {
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  g_promotion_adopt_parallel_active.fetch_sub(1);
  return promotion_adopt_success_for_test(preserve_dir, bundle, token_result);
}
...
TEST(PreservedTrxTempTable,
     OwnershipClaimsExcludeUnrelatedNoRedoUndoPages) {
  const std::string token = "token42";
  ...
  descriptor.no_redo_undo_rseg_space_id = 4294967294U;
  descriptor.no_redo_undo_rseg_page_no = 10;
  descriptor.no_redo_undo_rseg_slot = 3;
  descriptor.no_redo_update_undo.present = true;
  descriptor.no_redo_update_undo.undo_slot = 7;
  descriptor.no_redo_update_undo.hdr_page_no = 20;
  descriptor.no_redo_update_undo.last_page_no = 20;
  descriptor.no_redo_update_undo.top_page_no = 20;
  descriptor.no_redo_update_undo.hdr_offset = 128;
  descriptor.no_redo_update_undo.top_offset = 256;
  descriptor.no_redo_update_undo.top_undo_no = 100;

  auto add_page = [&descriptor](trx_preserve_temp_no_redo_undo_page_kind kind,
                                uint32_t page_no) {
    trx_preserve_temp_no_redo_undo_page_image image;
    image.kind = kind;
    image.page_no = page_no;
    image.bytes.assign(descriptor.page_size,
                       static_cast<unsigned char>(page_no & 0xff));
    if (kind == trx_preserve_temp_no_redo_undo_page_kind::UNDO_HEADER) {
      unsigned char *page = image.bytes.data();
      const size_t list_base =
          kTestTrxUndoSegHdr + kTestTrxUndoPageList;
      const size_t node_base =
          kTestTrxUndoPageHdr + kTestTrxUndoPageNode;
      test_mach_write_to_4(page + list_base + kTestFlstLen, 1);
      test_mach_write_to_4(
          page + list_base + kTestFlstFirst + kTestFilAddrPage, page_no);
      test_mach_write_to_2(
          page + list_base + kTestFlstFirst + kTestFilAddrByte, node_base);
      test_mach_write_to_4(
          page + list_base + kTestFlstLast + kTestFilAddrPage, page_no);
      test_mach_write_to_2(
          page + list_base + kTestFlstLast + kTestFilAddrByte, node_base);
      test_mach_write_to_4(
          page + node_base + kTestFlstPrev + kTestFilAddrPage, kTestFilNull);
      test_mach_write_to_2(page + node_base + kTestFlstPrev + kTestFilAddrByte,
                           0);
      test_mach_write_to_4(
          page + node_base + kTestFlstNext + kTestFilAddrPage, kTestFilNull);
      test_mach_write_to_2(page + node_base + kTestFlstNext + kTestFilAddrByte,
                           0);
    }
    descriptor.no_redo_undo_pages.push_back(std::move(image));
  };
  add_page(trx_preserve_temp_no_redo_undo_page_kind::RSEG_HEADER, 10);
  add_page(trx_preserve_temp_no_redo_undo_page_kind::UNDO_HEADER, 20);
  add_page(trx_preserve_temp_no_redo_undo_page_kind::UNDO_LOG, 21);

  Preserved_temp_table_undo_descriptor undo;
  undo.source_space_id = descriptor.source_space_id;
  undo.blob_name = token + ".tempts.42.undo";
  undo.size = 4096;
  undo.no_redo_undo_rseg_space_id = descriptor.no_redo_undo_rseg_space_id;
  ...
  std::set<uint32_t> exclusive_pages;
  for (const Preserved_temp_table_ownership_claim &claim :
    ...
    if (claim.page_role ==
        ...
        claim.page_role == trx_preserve_temp_no_redo_undo_page_kind::UNDO_LOG) {
      exclusive_pages.insert(claim.page_no);
    }
  }

  EXPECT_EQ(1U, exclusive_pages.size());
  EXPECT_TRUE(exclusive_pages.count(20));
  EXPECT_FALSE(exclusive_pages.count(21));
}

TEST(PreservedTrxTempTable,
     NativeOwnedNoRedoUndoUsesHistoryButSkipsCache) {
  EXPECT_FALSE(
      trx_preserve_temp_no_redo_undo_reconnect_mode_skips_history_for_test(
          true));
  EXPECT_TRUE(trx_preserve_temp_no_redo_undo_reconnect_mode_skips_cache_for_test(
      true));
...
TEST(PreservedTrxTransfer, ArtifactDecisionUsesArtifactModeAsSourceMode) {
...
class Transfer_codec_context_guard {
 public:
  Transfer_codec_context_guard() {
    ...
    preserve_trx_transfer_set_source_lsn_provider_for_unit_test(
        test_transfer_source_lsn_provider);
  }
...
TEST(PreservedTrxTransfer, ConfiguredFrameSinkUsesCredentialSecretFile) {
  char secret_path[FN_REFLEN];
  File secret_file = create_temp_file(secret_path, nullptr,
                                      "preserve-transfer-secret", 0600,
                                      KEEP_FILE, MYF(0));
...
TEST(PreservedTrxTransfer, ReceiverPolicyUsesConfiguredEndpointIdentity) {
  const std::string saved_server_uuid = server_uuid;
  const char *saved_server_uuid_ptr = server_uuid_ptr;
  std::snprintf(server_uuid, UUID_LENGTH + 1, "%s", "target-uuid");
  ...
  preserve_trx_transfer_receiver_enable = old_receiver_enable;
  std::snprintf(server_uuid, UUID_LENGTH + 1, "%s",
                saved_server_uuid.c_str());
  server_uuid_ptr = saved_server_uuid_ptr;
}
...
TEST(PreservedTrxLockWarmcopy,
     EmptyPhase1RecordPayloadDoesNotSeedRecordCandidate) {
  ...
  EXPECT_FALSE(participant.phase1_record_prebuilt_blob_for_thread(42, &blob));
  EXPECT_FALSE(participant.record_locks_seeded_in_phase1_for_unit_test(42));
...
class PreserveSnapshotTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override {
    preserve_trx_transfer_set_receiver_staged_prewarm_delay_ms_for_unit_test(0);
  ...
  bool wait_for_standby_artifact(
      Local_file_preserved_trx_carrier *carrier, const std::string &token,
      Preserved_trx_carrier_token_state *final_state = nullptr,
      uint attempts = 200) {
    for (uint attempt = 0; attempt < attempts; ++attempt) {
      ...
      if (state.snapshot && state.standby_pending) {
    ...
    if (final_state != nullptr) {
      EXPECT_EQ(Preserved_trx_carrier_status::OK,
                carrier->token_state(token, final_state));
    }
    return false;
  }

  bool wait_for_epoch_ready(const std::string &epoch_id,
                            size_t expected_ready_tokens,
    ...
    for (uint attempt = 0; attempt < attempts; ++attempt) {
      ...
      if (status == Preserve_trx_promotion_adopt_status::OK &&
          current.ready_tokens.size() == expected_ready_tokens) {
...
TEST_F(PreserveSnapshotTest,
       LockWarmcopyMaterializesQuiescedRecordBlobForTransferCatchup) {
  lock_warmcopy_reset_for_unit_test();
  ...
  artifact.record_locks_payload =
      record_locks_payload() + "quiesced-final-catchup-record";
  artifact.record_lock_count = 1;
  artifact.mdl_descriptors_payload = empty_mdl_descriptors_payload();
  participant.set_artifact_for_thread_for_unit_test(7001, artifact);

  PrebuiltRecordLocksBlob blob;
  EXPECT_EQ(Preserve_trx_lock_warmcopy_record_blob_status::OK,
            participant.ensure_quiesced_record_prebuilt_blob_for_thread(7001,
                                                                        &blob));
  EXPECT_EQ(kPreservedTrxBlobRecordLocks, blob.name);
  EXPECT_EQ(artifact.record_locks_payload.size(), blob.size);

  PrebuiltRecordLocksBlob lookup;
  EXPECT_TRUE(
      participant.phase1_record_prebuilt_blob_for_thread(7001, &lookup));
  EXPECT_EQ(blob.warmcopy_id, lookup.warmcopy_id);
...
TEST_F(PreserveSnapshotTest,
       StandbyProjectionExistsRequiresSnapshotAndMarker) {
  Local_file_preserved_trx_carrier carrier(m_dir);
  bool exists = true;

  EXPECT_EQ(Preserved_trx_carrier_status::OK,
            carrier.standby_projection_exists("901", &exists));
  EXPECT_FALSE(exists);

  write_file(m_dir + "901.standby_pending", "standby_pending\n");
  exists = true;
  EXPECT_EQ(Preserved_trx_carrier_status::OK,
            carrier.standby_projection_exists("901", &exists));
  EXPECT_FALSE(exists);

  write_file(m_dir + "901.bin", "snapshot\n");
  EXPECT_EQ(Preserved_trx_carrier_status::OK,
            carrier.standby_projection_exists("901", &exists));
  EXPECT_TRUE(exists);
}
...
TEST_F(PreserveSnapshotTest,
       PromotionExecuteAdoptRejectsMissingReadyCacheRequirement) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "916";
  Preserved_trx_bundle bundle;
...
TEST_F(PreserveSnapshotTest, PromotionRejectsMarkerOnlyEpochWithoutFact) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "923";
  Preserved_trx_bundle bundle;
  ...
  const std::string epoch_dir = m_dir + ".transfer/epoch-marker-only";
  ASSERT_EQ(0, my_mkdir((m_dir + ".transfer").c_str(), 0700, MYF(0)));
  ASSERT_EQ(0, my_mkdir(epoch_dir.c_str(), 0700, MYF(0)));
  write_file(epoch_dir + "/epoch.commit",
             "PTRXFER_COMMIT\nepoch-marker-only\nsource-uuid\ntarget-uuid\n");
  ASSERT_FALSE(
      preserve_trx_transfer_epoch_committed(m_dir, "epoch-marker-only"));

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-marker-only";
...
TEST_F(PreserveSnapshotTest, PromotionRejectsEpochFactTokenSetMismatch) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "925";
  Preserved_trx_bundle bundle;
  ...
  const std::string epoch_dir = m_dir + ".transfer/epoch-token-mismatch";
  ASSERT_EQ(0, my_mkdir((m_dir + ".transfer").c_str(), 0700, MYF(0)));
  ASSERT_EQ(0, my_mkdir(epoch_dir.c_str(), 0700, MYF(0)));
  write_file(epoch_dir + "/epoch.commit",
             "PTRXFER_COMMIT\nepoch-token-mismatch\nsource-uuid\ntarget-uuid\n");
  ...
  write_file(epoch_dir + "/epoch.fact", encoded_fact);
  ASSERT_TRUE(
      preserve_trx_transfer_epoch_committed(m_dir, "epoch-token-mismatch"));

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-token-mismatch";
  request.tokens.push_back(925);
  request.require_epoch_committed = true;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = false;
...
TEST_F(PreserveSnapshotTest, PromotionEmptyRequestUsesEpochFactTokenSet) {
  preserve_trx_set_enable_value(true);
  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);

  for (const char *token : {"931", "932"}) {
    Preserve_snapshot_metadata meta = metadata();
    meta.token = token;
    Preserved_trx_bundle bundle;
    Preserved_trx_bundle_build_input input;
  ...
  const std::string epoch_dir = m_dir + ".transfer/epoch-token-subset";
  ASSERT_EQ(0, my_mkdir((m_dir + ".transfer").c_str(), 0700, MYF(0)));
  ASSERT_EQ(0, my_mkdir(epoch_dir.c_str(), 0700, MYF(0)));
  write_file(epoch_dir + "/epoch.commit",
             "PTRXFER_COMMIT\nepoch-token-subset\nsource-uuid\ntarget-uuid\n");
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_epoch_fact(fact, &encoded_fact));
  write_file(epoch_dir + "/epoch.fact", encoded_fact);
  ASSERT_TRUE(preserve_trx_transfer_epoch_committed(m_dir,
                                                    "epoch-token-subset"));

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-token-subset";
  request.require_epoch_committed = true;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = false;
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_adopt_standby_pending_all_for_promotion(
                m_dir, request, &result));
  ASSERT_EQ(1U, result.seen_tokens.size());
  EXPECT_EQ(931U, result.seen_tokens[0]);
  EXPECT_EQ(1U, result.skipped_count);
  EXPECT_EQ(0U, result.abandoned_count);
  preserve_trx_set_enable_value(true);
}
...
TEST_F(PreserveSnapshotTest,
       PromotionStrictOnlineGateUsesEpochFactWithoutStandbyProjection) {
  preserve_trx_set_enable_value(true);
  preserved_trx_promotion_ready_cache_clear_for_unit_test();
  const uint64_t transfer_token = 936;
  const std::string epoch_id = "epoch-online-no-projection";
  ...
  const std::string epoch_dir = m_dir + ".transfer/" + epoch_id;
  ASSERT_EQ(0, my_mkdir((m_dir + ".transfer").c_str(), 0700, MYF(0)));
  ASSERT_EQ(0, my_mkdir(epoch_dir.c_str(), 0700, MYF(0)));
  write_file(epoch_dir + "/epoch.commit",
             "PTRXFER_COMMIT\n" + epoch_id + "\nsource-uuid\n" +
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_epoch_fact(fact, &encoded_fact));
  write_file(epoch_dir + "/epoch.fact", encoded_fact);
  ASSERT_TRUE(preserve_trx_transfer_epoch_committed(m_dir, epoch_id));

  preserved_trx_promotion_ready_cache_put_bundle_for_unit_test(
      m_dir, epoch_id, transfer_token,
      Preserve_trx_promotion_ready_state::PREWARMED_PENDING_FINAL_FACT, 0,
      bundle);
  ...
  ASSERT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_promotion_bind_prewarmed_epoch_for_receiver(
                m_dir, epoch_id, {transfer_token}, &ready_tokens));
  ASSERT_EQ(1U, ready_tokens);

  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);
  Preserved_trx_carrier_listing listing;
  ASSERT_EQ(Preserve_snapshot_status::OK, store.list_tokens(&listing));
  ASSERT_TRUE(listing.snapshot_tokens.empty());
  ASSERT_TRUE(listing.standby_pending_tokens.empty());
  ...
  EXPECT_EQ(0U, result.abandoned_count);
  ASSERT_EQ(1U, result.seen_tokens.size());
  EXPECT_EQ(transfer_token, result.seen_tokens[0]);
  preserved_trx_promotion_ready_cache_clear_for_unit_test();
  preserve_trx_set_enable_value(true);
}
...
TEST_F(PreserveSnapshotTest, PromotionEpochCommitMarkerSatisfiesGate) {
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-committed", "source-uuid", server_uuid, bundle,
  ...
  ASSERT_TRUE(preserve_trx_transfer_epoch_committed(m_dir, manifest));
  ASSERT_EQ("PTRXFER_COMMIT\nepoch-committed\nsource-uuid\n" +
                std::string(server_uuid) + "\n",
  ...
  EXPECT_EQ(transfer_token, ready_summary.ready_tokens[0]);

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = manifest.epoch_id;
  request.require_epoch_committed = true;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = true;
...
TEST_F(PreserveSnapshotTest, PromotionRejectsReadyCacheFactDigestDrift) {
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-fact-drift", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_epoch_fact(fact, &encoded_fact));
  write_file(m_dir + ".transfer/epoch-fact-drift/epoch.fact", encoded_fact);
  ASSERT_TRUE(preserve_trx_transfer_epoch_committed(m_dir, manifest.epoch_id));
...
TEST_F(PreserveSnapshotTest, PromotionGateRejectsMoreThanBatchTokens) {
  preserve_trx_set_enable_value(true);
  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);
  preserved_trx_promotion_ready_cache_clear_for_unit_test();

  for (const char *token : {"931", "932", "933", "934"}) {
    Preserve_snapshot_metadata meta = metadata();
    meta.token = token;
    Preserved_trx_bundle bundle;
    Preserved_trx_bundle_build_input input;
    ...
    ASSERT_EQ(Preserve_snapshot_status::OK,
              store.write_standby_pending(std::move(bundle), 300,
                                          &written_metadata));
    preserved_trx_promotion_ready_cache_put_for_unit_test(
        m_dir, "epoch-1", std::stoull(token),
        Preserve_trx_promotion_ready_state::READY, 0);
  }

  g_promotion_adopt_counting_success_calls = 0;
  preserved_trx_set_promotion_adopt_executor_for_unit_test(
      promotion_adopt_counting_success_for_test);

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-1";
  request.tokens = {931, 932, 933, 934};
  request.require_epoch_committed = false;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = true;
  request.execute_adopt = true;
  request.gate_batch_tokens = 3;

  Preserve_trx_promotion_adopt_result result;
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::TOO_MANY_PROMOTION_TOKENS,
            preserved_trx_adopt_standby_pending_all_for_promotion(
                m_dir, request, &result));
  EXPECT_EQ(0, g_promotion_adopt_counting_success_calls);
  EXPECT_EQ(0U, result.adopted_count);
  EXPECT_EQ(0U, result.abandoned_count);
  EXPECT_NE(std::string::npos, result.message.find("too_many_promotion_tokens"));
...
TEST_F(PreserveSnapshotTest,
       StrictPhysicalFenceAcceptsDistinctTokenDigestsWithEpochCommitment) {
  preserve_trx_set_enable_value(true);
  preserve_trx_resource_manager_reset_for_unit_test();
  auto proof = make_test_physical_fence_proof(
      Preserve_trx_physical_consistency_mode::
  ...
  Preserve_trx_physical_promotion_gate_request request;
  request.epoch_id = "strict-distinct-digest-epoch";
  request.operation_deadline_us = my_micro_time() + 1000000;
  ...
  for (uint64_t i = 1; i <= 2; ++i) {
    ...
    key.source_uuid = proof.source_lineage_uuid;
    key.epoch_id = request.epoch_id;
    key.token = "strict-distinct-" + std::to_string(i);
    key.target_boot_incarnation = proof.target_boot_incarnation;
    request.tokens.push_back(key);

    auto facts = make_final_token_facts(proof.source_fence_lsn);
    facts.epoch_fact_digest = proof.epoch_fact_digest;
    facts.final_lock_generation_digest.assign(64, i == 1 ? '1' : '4');
    facts.page_layout_digest.assign(64, i == 1 ? '2' : '5');
    facts.dictionary_generation_digest.assign(64, i == 1 ? '3' : '6');
    facts.target_boot_incarnation = proof.target_boot_incarnation;
    facts.canonical_digest.clear();
    ASSERT_TRUE(preserved_trx_finalize_token_facts(&facts));
...
TEST_F(PreserveSnapshotTest, PromotionGateUsesWorkerCountForReadyAdopt) {
  ...
  for (const char *token : {"941", "942", "943"}) {
    Preserve_snapshot_metadata meta = metadata();
    meta.token = token;
    Preserved_trx_bundle bundle;
    Preserved_trx_bundle_build_input input;
    ...
    ASSERT_EQ(Preserve_snapshot_status::OK,
              build_preserved_trx_bundle(input, &bundle));
    Preserved_trx_bundle ready_bundle = bundle;
    Preserve_snapshot_metadata written_metadata;
    ASSERT_EQ(Preserve_snapshot_status::OK,
              store.write_standby_pending(std::move(bundle), 300,
                                          &written_metadata));
    preserved_trx_promotion_ready_cache_put_bundle_for_unit_test(
        m_dir, "epoch-1", std::stoull(token),
        Preserve_trx_promotion_ready_state::READY, 0, ready_bundle);
  }

  g_promotion_adopt_parallel_active.store(0);
  g_promotion_adopt_parallel_max_active.store(0);
  g_promotion_adopt_parallel_calls.store(0);
  preserved_trx_set_promotion_adopt_executor_for_unit_test(
      promotion_adopt_parallel_probe_for_test);

  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-1";
  request.tokens = {941, 942, 943};
  request.require_epoch_committed = false;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = true;
  request.execute_adopt = true;
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_adopt_standby_pending_all_for_promotion(
                m_dir, request, &result));
  EXPECT_EQ(3U, result.adopted_count);
  EXPECT_EQ(3, g_promotion_adopt_parallel_calls.load());
  EXPECT_GT(g_promotion_adopt_parallel_max_active.load(), 1);
  EXPECT_GT(result.p50_worker_elapsed_us, 0U);
  EXPECT_GT(result.p95_worker_elapsed_us, 0U);
  EXPECT_LE(result.p50_worker_elapsed_us, result.max_worker_elapsed_us);
  EXPECT_LE(result.p95_worker_elapsed_us, result.max_worker_elapsed_us);
  EXPECT_EQ(result.p50_worker_elapsed_us,
            preserve_trx_promotion_gate_p50_worker_elapsed_us_status());
  EXPECT_EQ(result.p95_worker_elapsed_us,
...
TEST_F(PreserveSnapshotTest, PromotionGateReadyCacheMissDoesNotAdopt) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "919";
  Preserved_trx_bundle bundle;
  ...
  EXPECT_EQ(0U, result.adopted_count);
  EXPECT_EQ(1U, result.abandoned_count);
  EXPECT_EQ(1U, result.ready_cache_miss_count);
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_EQ(919U, result.token_results[0].token);
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            result.token_results[0].status);
...
TEST_F(PreserveSnapshotTest,
       PromotionGateRejectsRecordLocksWithoutPagePrewarmProof) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "944";
  ...
  preserved_trx_promotion_ready_cache_clear_for_unit_test();
  preserved_trx_promotion_ready_cache_put_bundle_for_unit_test(
      m_dir, "epoch-1", 944, Preserve_trx_promotion_ready_state::READY, 0,
      bundle);
  g_promotion_adopt_counting_success_calls = 0;
  preserved_trx_set_promotion_adopt_executor_for_unit_test(
      promotion_adopt_counting_success_for_test);
  ...
  EXPECT_EQ(0U, result.adopted_count);
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_EQ(944U, result.token_results[0].token);
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            result.token_results[0].status);
  EXPECT_EQ("record-lock pages were not prewarmed",
            result.token_results[0].reason);
...
TEST_F(PreserveSnapshotTest, PromotionGateAllowsLargeResidentRecordLockProof) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "945";
  meta.record_locks_payload = record_locks_payload();
  ...
  preserved_trx_promotion_ready_cache_put_bundle_with_record_lock_proof_for_unit_test(
      m_dir, "epoch-1", 945, Preserve_trx_promotion_ready_state::READY, 0,
      bundle, 4096, 4096, 4096);
  g_promotion_adopt_counting_success_calls = 0;
  preserved_trx_set_promotion_adopt_executor_for_unit_test(
      promotion_adopt_counting_success_for_test);
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_adopt_standby_pending_all_for_promotion(
                m_dir, request, &result));
  EXPECT_EQ(1, g_promotion_adopt_counting_success_calls);
  EXPECT_EQ(1U, result.adopted_count);
  EXPECT_EQ(0U, result.abandoned_count);
  EXPECT_TRUE(result.token_results.empty());
...
TEST_F(PreserveSnapshotTest, PromotionReadyRecordLockMetricsRequireResidency) {
  EXPECT_TRUE(
      preserved_trx_promotion_record_lock_pages_gate_ready_for_unit_test(
          true, 0, 0, 0));
  EXPECT_FALSE(
      preserved_trx_promotion_record_lock_pages_gate_ready_for_unit_test(
          false, 10, 10, 10));
...
TEST_F(PreserveSnapshotTest, PromotionPrewarmStatusSummarizesRecordLockPages) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "947";
  meta.record_locks_payload = record_locks_payload();
  ...
  preserved_trx_promotion_ready_cache_put_bundle_with_record_lock_residency_for_unit_test(
      m_dir, "epoch-1", 947, Preserve_trx_promotion_ready_state::READY, 0,
      bundle, 10, 10, 7, 1, 10);

  EXPECT_EQ(10U, preserve_trx_promotion_prewarm_record_lock_page_count_status());
  EXPECT_EQ(7U,
...
TEST_F(PreserveSnapshotTest,
       StartupRecordLockPrewarmRequiresResidentPageProof) {
  EXPECT_TRUE(
      preserved_trx_startup_record_lock_pages_prewarmed_for_unit_test(0, 0, 0));
  EXPECT_FALSE(
      preserved_trx_startup_record_lock_pages_prewarmed_for_unit_test(10, 9, 9));
  EXPECT_FALSE(preserved_trx_startup_record_lock_pages_prewarmed_for_unit_test(
      10, 10, 9));
  EXPECT_TRUE(preserved_trx_startup_record_lock_pages_prewarmed_for_unit_test(
      10, 10, 10));
}

TEST_F(PreserveSnapshotTest,
       PromotionPrewarmResidencyWaitsUntilAllRecordLockPagesResident) {
  uint64_t final_resident_pages = 0;
  uint64_t sample_count = 0;

  EXPECT_TRUE(
      preserved_trx_promotion_record_lock_pages_wait_for_residency_for_unit_test(
          true, 3, 3, {0, 2, 3}, &final_resident_pages, &sample_count));
  EXPECT_EQ(3U, final_resident_pages);
  EXPECT_EQ(3U, sample_count);
}

TEST_F(PreserveSnapshotTest,
       PromotionPrewarmResidencyWaitStopsWhenRecordLockPagesStayCold) {
  uint64_t final_resident_pages = 0;
  uint64_t sample_count = 0;
...
TEST_F(PreserveSnapshotTest,
  ...
  EXPECT_EQ(0U, result.adopted_count);
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_EQ(946U, result.token_results[0].token);
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            result.token_results[0].status);
  EXPECT_EQ("record-lock pages are not resident",
            result.token_results[0].reason);
...
TEST_F(PreserveSnapshotTest,
  ...
  Local_file_preserved_trx_carrier read_carrier(m_dir);
  Preserved_trx_store read_store(&read_carrier);
  Preserved_trx_carrier_listing listing;
  ASSERT_EQ(Preserve_snapshot_status::OK, read_store.list_tokens(&listing));
...
TEST_F(PreserveSnapshotTest, PromotionPrewarmBatchBuildsReadyRecords) {
  preserve_trx_set_enable_value(true);
  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);
  for (const char *token : {"931", "932"}) {
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_promotion_prewarm_standby_pending_tokens(
                m_dir, "epoch-1", {931, 932}, 444, 2, &result));
  EXPECT_EQ(2U, result.token_results.size());
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_promotion_ready_summary_for_epoch(
                m_dir, "epoch-1", &summary));
  EXPECT_EQ(444U, summary.max_required_apply_lsn);
  EXPECT_EQ(2U, summary.ready_tokens.size());
  EXPECT_TRUE(std::find(summary.ready_tokens.begin(), summary.ready_tokens.end(),
                        931) != summary.ready_tokens.end());
  EXPECT_TRUE(std::find(summary.ready_tokens.begin(), summary.ready_tokens.end(),
                        932) != summary.ready_tokens.end());
...
TEST_F(PreserveSnapshotTest, PromotionPrewarmRejectsMarkerOnlyStandbyToken) {
  ...
  ASSERT_EQ(1U, summary.token_results.size());
  EXPECT_EQ(922U, summary.token_results[0].token);
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::CORRUPT_ARTIFACT,
            summary.token_results[0].status);
  EXPECT_NE(std::string::npos,
            summary.token_results[0].reason.find("bundle load failed"));

  Preserve_trx_promotion_adopt_result result;
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::CORRUPT_ARTIFACT,
            preserved_trx_promotion_prewarm_standby_pending_tokens(
                m_dir, "epoch-1", {922}, 0, 1, &result));
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_NE(std::string::npos,
            result.token_results[0].reason.find("bundle load failed"));
  preserve_trx_set_enable_value(true);
}
...
TEST_F(PreserveSnapshotTest, PromotionDebugApplyProviderEnablesSimulatorGate) {
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            store.write_standby_pending(std::move(bundle), 300,
                                        &written_metadata));
  preserved_trx_promotion_ready_cache_put_for_unit_test(
      m_dir, "epoch-1", 912, Preserve_trx_promotion_ready_state::READY, 250);

  preserved_trx_set_promotion_apply_state_provider_for_unit_test(nullptr);
  DBUG_SET("+d,preserve_trx_promotion_debug_apply_reached");
  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-1";
  request.tokens.push_back(912);
  request.required_apply_lsn = 250;
  request.require_epoch_committed = false;
  request.require_apply_barrier = true;
  request.require_promotion_ready_cache = true;
...
TEST_F(PreserveSnapshotTest, PromotionGateStatusReflectsLatestGateResult) {
  preserve_trx_set_enable_value(true);
  preserved_trx_promotion_ready_cache_clear_for_unit_test();
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "913";
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            store.write_standby_pending(std::move(bundle), 300,
                                        &written_metadata));
  preserved_trx_promotion_ready_cache_put_for_unit_test(
      m_dir, "epoch-1", 913, Preserve_trx_promotion_ready_state::READY, 250);
  ...
  EXPECT_EQ(result.elapsed_us, preserve_trx_promotion_gate_elapsed_us_status());
  EXPECT_EQ(1U, preserve_trx_promotion_gate_token_count_status());
  EXPECT_EQ(0U, preserve_trx_promotion_gate_adopted_count_status());
  EXPECT_EQ(0U, preserve_trx_promotion_gate_abandoned_count_status());
  EXPECT_EQ(1U, preserve_trx_promotion_gate_skipped_count_status());
  EXPECT_EQ(result.max_worker_elapsed_us,
            preserve_trx_promotion_gate_max_worker_elapsed_us_status());
  EXPECT_EQ(static_cast<ulonglong>(Preserve_trx_promotion_adopt_status::OK),
...
TEST_F(PreserveSnapshotTest,
       PromotionExplicitApplyProviderPassesBarrierBeforeReadyCache) {
  preserve_trx_set_enable_value(true);
  Preserve_snapshot_metadata meta = metadata();
  meta.token = "914";
  Preserved_trx_bundle bundle;
  ...
  preserved_trx_set_promotion_apply_state_provider(
      promotion_apply_provider_reached_for_test);
  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = "epoch-1";
  request.tokens.push_back(914);
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            result.token_results[0].status);
  preserved_trx_set_promotion_apply_state_provider(nullptr);
  preserve_trx_set_enable_value(true);
}
...
TEST_F(PreserveSnapshotTest,
       ReceiverProjectionUsesAlreadyInstalledPrebuiltExternalBlob) {
  const std::string binlog_payload = "receiver-installed-binlog-cache";
  const std::string token = "604";

  PrebuiltBinlogCacheBlob prebuilt;
  prebuilt.warmcopy_id = "receiver-sealed-object";
  prebuilt.warmcopy_epoch = 77;
  prebuilt.size = binlog_payload.length();
  SHA_EVP256(reinterpret_cast<const unsigned char *>(binlog_payload.data()),
             binlog_payload.length(), prebuilt.digest.data());
  prebuilt.metadata.gtid_next = "AUTOMATIC";
  prebuilt.metadata.event_counter = 11;
  prebuilt.metadata.with_rbr = true;
  prebuilt.metadata.with_start = true;
  prebuilt.metadata.with_content = true;
  ...
  write_file(m_dir + token + ".binlog_cache", binlog_payload);
  ...
  Preserved_trx_store read_store(&read_carrier);
  Preserved_trx_bundle out;
  ASSERT_EQ(Preserve_snapshot_status::OK, read_store.read(token, true, &out));
  EXPECT_EQ(binlog_payload, out.metadata.binlog_cache_payload);
  Preserved_trx_carrier_token_state state;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(token, &state));
...
TEST_F(PreserveSnapshotTest, LocalFileStoreListsTmpArtifactsOnlyAsStale) {
  const std::string token = metadata().token;
  write_file(m_dir + token + ".binlog_cache.tmp", "binlog-cache-tmp");
  write_file(m_dir + token + ".blob.audit.tmp", "generic-blob-tmp");
  write_file(m_dir + token + ".tainted.tmp", "tainted-tmp");
  write_file(m_dir + token + ".tempts.77.image.tmp", "image-tmp");
  ...
  EXPECT_EQ(1U, listing.stale_tmp_tokens.count(token));
  EXPECT_EQ(0U, listing.snapshot_tokens.count(token));
  EXPECT_EQ(0U, listing.external_blob_tokens.count(token));
  EXPECT_EQ(0U, listing.tainted_tokens.count(token));
  EXPECT_EQ(0U, listing.temp_sidecar_tokens.count(token));
}
...
TEST_F(PreserveSnapshotTest, LocalFileStoreShardsStandbyPendingMarker) {
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = metadata();
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  const std::string token = input.metadata.token;
  ...
  const std::string shard_dir =
      m_dir + "blob_shards" + FN_DIRSEP + token.substr(0, 1) + FN_DIRSEP;
  MY_STAT stat_area;
  EXPECT_EQ(nullptr,
            my_stat((m_dir + token + ".standby_pending").c_str(), &stat_area,
                    MYF(0)));
  EXPECT_NE(nullptr,
            my_stat((shard_dir + token + ".standby_pending").c_str(),
                    &stat_area, MYF(0)));
  ...
  Preserved_trx_carrier_token_state state;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            read_carrier.token_state(token, &state));
  EXPECT_TRUE(state.snapshot);
  EXPECT_TRUE(state.standby_pending);

  bool exists = false;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            read_carrier.standby_projection_exists(token, &exists));
  EXPECT_TRUE(exists);
}

TEST_F(PreserveSnapshotTest,
       LocalFileStoreCanFsyncSingleShardedStandbyProjectionDirectory) {
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = metadata();
  input.metadata.token = "902";
  input.metadata.mdl_descriptors_payload = empty_mdl_descriptors_payload();
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  const std::string token = input.metadata.token;
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            preserve_trx_fsync_default_store_token_directory(m_dir, token));
...
TEST_F(PreserveSnapshotTest, StandbyPendingStoreRequiresMainPreserveEnable) {
  const bool saved_enable = preserve_trx_enable;
  preserve_trx_set_enable_value(false);
...
TEST(PreservedTrxTransfer, FrameCodecRoundTripsPromotionControlFrames) {
  Preserve_trx_transfer_frame prewarm;
  prewarm.type = Preserve_trx_transfer_frame_type::PROMOTION_PREWARM_TOKEN;
  prewarm.sequence = 13;
  prewarm.epoch_id = "epoch-1";
  prewarm.token = 101;
  prewarm.chunk_offset = 88;

  std::string encoded_prewarm;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_frame(prewarm, &encoded_prewarm));
  Preserve_trx_transfer_frame decoded_prewarm;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(encoded_prewarm,
                                               &decoded_prewarm));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::PROMOTION_PREWARM_TOKEN,
            decoded_prewarm.type);
  EXPECT_EQ(13U, decoded_prewarm.sequence);
  EXPECT_EQ("epoch-1", decoded_prewarm.epoch_id);
  EXPECT_EQ(101U, decoded_prewarm.token);
  EXPECT_EQ(88U, decoded_prewarm.chunk_offset);

  Preserve_trx_transfer_frame gate;
  gate.type = Preserve_trx_transfer_frame_type::PROMOTION_GATE_EPOCH;
  gate.sequence = 14;
  gate.epoch_id = "epoch-1";
  gate.token = 0;
  gate.chunk_offset = 88;

  std::string encoded_gate;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_frame(gate, &encoded_gate));
  Preserve_trx_transfer_frame decoded_gate;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(encoded_gate, &decoded_gate));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::PROMOTION_GATE_EPOCH,
            decoded_gate.type);
  EXPECT_EQ(14U, decoded_gate.sequence);
  EXPECT_EQ("epoch-1", decoded_gate.epoch_id);
  EXPECT_EQ(0U, decoded_gate.token);
  EXPECT_EQ(88U, decoded_gate.chunk_offset);
}
...
TEST_F(PreserveSnapshotTest,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-1", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  ...
  EXPECT_EQ(manifest.source_server_uuid, fact.source_server_uuid);
  EXPECT_EQ(manifest.target_server_uuid, fact.target_server_uuid);
  ASSERT_EQ(1U, fact.tokens.size());
  ...
  ASSERT_FALSE(fact.tokens[0].objects.empty());
  EXPECT_EQ(manifest.objects[0].object_id, fact.tokens[0].objects[0].object_id);
  EXPECT_EQ(manifest.objects[0].digest, fact.tokens[0].objects[0].digest);
  ...
  ASSERT_TRUE(wait_for_epoch_ready(manifest.epoch_id, 1, &ready_summary));
  ASSERT_EQ(1U, ready_summary.ready_tokens.size());
  EXPECT_EQ(manifest.token, ready_summary.ready_tokens[0]);
  EXPECT_EQ(manifest.source_epoch_commit_lsn,
            ready_summary.max_required_apply_lsn);
  EXPECT_EQ(auto_prewarm_tokens_before + 1,
            preserve_trx_transfer_receiver_auto_prewarm_tokens_status());
  EXPECT_EQ(auto_prewarm_ready_before + 1,
            preserve_trx_transfer_receiver_auto_prewarm_ready_tokens_status());
  EXPECT_EQ(
      auto_prewarm_not_ready_before,
      preserve_trx_transfer_receiver_auto_prewarm_not_ready_tokens_status());
  EXPECT_EQ(static_cast<uint64_t>(Preserve_trx_promotion_adopt_status::OK),
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverRejectsFrameTargetThatIsNotLocalServerUuid) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1020;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-target-mismatch", "source-uuid",
                "bbbbbbbb-cccc-dddd-eeee-ffffffffffff", bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid",
  ...
  ASSERT_EQ(1U, ready_summary.corrupt_tokens.size());
  EXPECT_EQ(manifest.token, ready_summary.corrupt_tokens[0]);
}
...
TEST_F(PreserveSnapshotTest,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-seal-prewarm", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverCommitEpochFallbackBindsWhenSealPrewarmDelayed) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1024;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-delayed-seal-prewarm", "source-uuid", server_uuid,
  ...
  preserve_trx_transfer_set_receiver_staged_prewarm_delay_ms_for_unit_test(500);
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverRecordLockObjectProofBindsReadyAfterStagedPrewarm) {
  Transfer_codec_context_guard codec_guard;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-record-proof-bind", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));

  const Preserve_trx_transfer_object_payload *record_object = nullptr;
  for (const Preserve_trx_transfer_object_payload &object : objects) {
    if (object.descriptor.object_id == kPreservedTrxBlobRecordLocks) {
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_object(transfer_token, record_object->descriptor));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(
                transfer_token, record_object->descriptor.object_id, 0,
                record_object->payload));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.seal_object(transfer_token,
                                record_object->descriptor.object_id));
  ...
  auto apply_new_frames = [&]() {
    std::vector<std::string> frames(sink.frames().begin() + consumed_frames,
                                    sink.frames().end());
    consumed_frames = sink.frames().size();
    return preserve_trx_transfer_handle_receiver_payload_batch(
        m_dir, frames, &store, &registry, 300, 3);
  };
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.begin_token_objects(manifest, true));
  for (const Preserve_trx_transfer_object_payload &object : objects) {
    if (object.descriptor.object_id == record_object->descriptor.object_id) {
      continue;
    }
    for (uint64_t offset = 0; offset < object.payload.length();
         offset += session.chunk_bytes()) {
      const size_t length = std::min<uint64_t>(
          session.chunk_bytes(), object.payload.length() - offset);
      ASSERT_EQ(Preserve_trx_transfer_status::OK,
                session.write_object_chunk(
                    transfer_token, object.descriptor.object_id, offset,
                    object.payload.substr(offset, length)));
    }
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              session.seal_object(transfer_token, object.descriptor.object_id));
  }
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  for (uint attempt = 0; attempt < 100; ++attempt) {
    ...
    if (preserved_trx_promotion_ready_summary_for_epoch(
            m_dir, manifest.epoch_id, &ready_summary) ==
            Preserve_trx_promotion_adopt_status::OK &&
        ready_summary.ready_tokens.size() == 1U) {
      break;
    }
    my_sleep(10000);
  }
  ...
  EXPECT_TRUE(strict_snapshot.record_lock_plan_owned);
  EXPECT_EQ(lock_facts.final_lock_generation_digest,
            strict_snapshot.facts.final_lock_generation_digest);
  EXPECT_EQ(lock_facts.page_layout_digest,
            strict_snapshot.facts.page_layout_digest);
  EXPECT_EQ(lock_facts.dictionary_generation_digest,
            strict_snapshot.facts.dictionary_generation_digest);
  EXPECT_EQ(lock_facts.bitmap_entries,
            strict_snapshot.facts.record_lock_bitmap_entries);
  EXPECT_EQ(lock_facts.bitmap_bits, strict_snapshot.facts.record_lock_bits);
  strict_registry.purge_epoch(strict_key.source_uuid, strict_key.epoch_id);
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverStaleRecordLockObjectProofRetriesStagedPrewarm) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1027;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  meta.record_locks_payload = record_locks_payload();
  Preserved_trx_bundle bundle;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-record-proof-stale", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK, apply_new_frames());
  preserve_trx_transfer_put_receiver_object_prewarm_proof_for_unit_test(
      m_dir, manifest, record_object->descriptor.object_id, 1, 0, 1, 1, 1);
  ...
  for (uint attempt = 0; attempt < 100; ++attempt) {
    if (preserve_trx_transfer_receiver_seal_prewarm_not_ready_tokens_status() >
            not_ready_before &&
        preserve_trx_transfer_receiver_record_object_prewarm_count_status() >=
  ...
  EXPECT_GT(preserve_trx_transfer_receiver_seal_prewarm_not_ready_tokens_status(),
            not_ready_before);
  EXPECT_GE(preserve_trx_transfer_receiver_record_object_prewarm_count_status(),
            record_object_prewarm_before + 1);
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverStagedPrewarmDoesNotColdImportRecordLocksWithoutProof) {
  Transfer_codec_context_guard codec_guard;
  preserved_trx_promotion_ready_cache_clear_for_unit_test();
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-record-proof-missing", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  ...
  Preserve_trx_transfer_receiver_registry registry;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            registry.declare_token(manifest.epoch_id, manifest.token,
                                   manifest.source_server_uuid,
                                   manifest.target_server_uuid));

  for (const Preserve_trx_transfer_object_payload &object : objects) {
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              registry.declare_object(manifest.epoch_id, manifest.token,
                                      object.descriptor));
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_stage_object_chunk(
                  ...
                  object.payload));
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_seal_staged_object(
                  m_dir, manifest, object.descriptor.object_id));
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              registry.mark_object_sealed(manifest.epoch_id, manifest.token,
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverCommitEpochFastBindsSealTimeReadyToken) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1023;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-fast-bind", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverAbortPurgesSealTimeStrictPreparedToken) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1027;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-strict-abort", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  abort.reason = "receiver abort before commit";
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
...
TEST_F(PreserveSnapshotTest,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-online-ready", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  bool projection_exists = true;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.standby_projection_exists(meta.token, &projection_exists));
...
TEST_F(PreserveSnapshotTest,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_bundles(
                "epoch-projection-metrics", "source-uuid", server_uuid,
                bundles, transfer_tokens, 7, &sink, &manifests));
  ...
  Preserve_trx_promotion_ready_summary ready_summary;
  ASSERT_TRUE(wait_for_epoch_ready("epoch-projection-metrics", 2,
                                   &ready_summary));
  std::set<uint64_t> ready_tokens(ready_summary.ready_tokens.begin(),
                                  ready_summary.ready_tokens.end());
  EXPECT_EQ(std::set<uint64_t>(transfer_tokens.begin(), transfer_tokens.end()),
            ready_tokens);

  EXPECT_EQ(bind_attempts_before + 1,
            preserve_trx_transfer_receiver_epoch_ready_bind_attempts_status());
  EXPECT_EQ(projection_count_before,
            preserve_trx_transfer_receiver_projection_publish_count_status());
  EXPECT_EQ(projection_total_before,
            preserve_trx_transfer_receiver_projection_publish_us_status());
  EXPECT_EQ(projection_lock_wait_before,
            preserve_trx_transfer_receiver_projection_lock_wait_us_status());
  EXPECT_EQ(
      projection_snapshot_before,
      preserve_trx_transfer_receiver_projection_snapshot_write_us_status());
  EXPECT_EQ(projection_marker_before,
            preserve_trx_transfer_receiver_projection_marker_write_us_status());
}

TEST_F(PreserveSnapshotTest, TransferReceiverBeginWritesDurableFrameSpool) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1021;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-spool", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
...
TEST_F(PreserveSnapshotTest, TransferReceiverReplaysDurableFrameSpool) {
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-spool-replay", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_replay_receiver_spool(
                m_dir, manifest.epoch_id, &store, &replayed_registry, 300));
  Preserve_trx_transfer_receiver_record replayed_record;
  ASSERT_TRUE(replayed_registry.lookup(manifest.epoch_id, manifest.token,
                                       &replayed_record));
  EXPECT_TRUE(replayed_registry.all_objects_sealed(manifest.epoch_id,
                                                  manifest.token));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, commit, &store, &replayed_registry, 300, nullptr));
  EXPECT_TRUE(preserve_trx_transfer_epoch_committed(m_dir, manifest));
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverReplaysCommittedSpoolIdempotentlyAndRebuildsReady) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1024;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-spool-replay-committed", "source-uuid", server_uuid,
                bundle, transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  EXPECT_EQ(manifest.token, ready_summary.ready_tokens[0]);
  EXPECT_TRUE(ready_summary.pending_tokens.empty());
  EXPECT_TRUE(ready_summary.corrupt_tokens.empty());
}

TEST_F(PreserveSnapshotTest, TransferReceiverReplayIgnoresTornTailFrame) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1023;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-spool-torn-tail", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  const std::string spool_path =
      m_dir + ".transfer/" + manifest.epoch_id + "/receiver.frames";
  File file = my_open(spool_path.c_str(), O_WRONLY | O_APPEND, MYF(0));
  ASSERT_GE(file, 0);
  const std::string torn_tail = "PTRXSPL1";
  ASSERT_EQ(torn_tail.size(),
            my_write(file, reinterpret_cast<const uchar *>(torn_tail.data()),
                     torn_tail.size(), MYF(0)));
  ASSERT_EQ(0, my_close(file, MYF(0)));
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverRejectsFrameSequenceGapBeforeStagingChunk) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 1023;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-sequence-gap", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  ...
  Preserve_trx_transfer_frame gap_chunk;
  gap_chunk.type = Preserve_trx_transfer_frame_type::OBJECT_CHUNK;
  gap_chunk.sequence = 3;
  gap_chunk.epoch_id = manifest.epoch_id;
  gap_chunk.token = manifest.token;
  gap_chunk.object_id = objects.front().descriptor.object_id;
  gap_chunk.chunk_payload = objects.front().payload;
  EXPECT_EQ(Preserve_trx_transfer_status::CORRUPT,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, gap_chunk, &store, &registry, 300, nullptr));

  const std::string part_path =
      m_dir + ".transfer/" + manifest.epoch_id + "/" +
      test_transfer_token_string(manifest.token) + "/" +
      objects.front().descriptor.object_id + ".part";
  MY_STAT stat_area;
  EXPECT_EQ(nullptr, my_stat(part_path.c_str(), &stat_area, MYF(0)));

  Preserve_trx_transfer_frame expected_chunk = gap_chunk;
  expected_chunk.sequence = 2;
  EXPECT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, expected_chunk, &store, &registry, 300, nullptr));
}
...
TEST_F(PreserveSnapshotTest,
       TransferPortableBundleUsesConfiguredCredentialContext) {
  Transfer_source_config_guard config;
  config.tcp("target-uuid", "127.0.0.1", 3307, "transfer_user",
             "transfer_credential");
  Transfer_credentials_guard credentials;
...
TEST_F(PreserveSnapshotTest,
       TransferPortableBundleRejectsDifferentCredentialSecret) {
  Transfer_source_config_guard config;
  config.tcp("target-uuid", "127.0.0.1", 3307, "transfer_user",
             "transfer_credential");
  Transfer_credentials_guard credentials;
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverRecordLockBlobPrewarmWaitsForPhysicalStandbyPages) {
  Transfer_codec_context_guard codec_guard;
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid", server_uuid);
  preserve_trx_set_enable_value(true);

  const uint64_t transfer_token = 940;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  meta.record_locks_payload = record_locks_payload();
  meta.mdl_descriptors_payload = empty_mdl_descriptors_payload();
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-record-lock-prewarm", "source-uuid", "target-uuid",
                bundle, transfer_token, &manifest, &objects));
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            preserved_trx_promotion_prewarm_standby_pending_tokens(
                m_dir, manifest.epoch_id, {transfer_token}, 0, 1, &result));
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            result.token_results[0].status);
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::READY_CACHE_NOT_READY,
            preserved_trx_promotion_ready_summary_for_epoch(
                m_dir, manifest.epoch_id, &summary));
  EXPECT_TRUE(summary.ready_tokens.empty());
  EXPECT_EQ(1U, summary.pending_tokens.size());
  EXPECT_TRUE(summary.corrupt_tokens.empty());
...
TEST_F(PreserveSnapshotTest,
  ...
  apply_objects(first_manifest, first_objects);
  ...
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(
                ...
                &second_state));
  EXPECT_FALSE(second_state.snapshot);
...
TEST_F(PreserveSnapshotTest,
  ...
  write_file(m_dir + ".transfer/epoch-partial-publish/epoch.commit",
             "stale-commit-marker\n");
  ...
  Preserve_trx_promotion_adopt_all_request request;
  request.epoch_id = first_manifest.epoch_id;
  request.tokens.push_back(first_manifest.token);
  request.require_epoch_committed = true;
  request.require_apply_barrier = false;
  request.require_promotion_ready_cache = true;
  request.execute_adopt = false;
  ...
  EXPECT_EQ(0U, result.adopted_count);
  ASSERT_EQ(1U, result.token_results.size());
  EXPECT_NE(Preserve_trx_promotion_adopt_status::OK,
            result.token_results[0].status);
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverPromotionPrewarmControlFrameBuildsReadyRecord) {
  preserve_trx_set_enable_value(true);
  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);
  ...
  EXPECT_EQ(Preserve_trx_promotion_adopt_status::OK,
            preserved_trx_promotion_ready_summary_for_epoch(
                m_dir, "epoch-1", &summary));
  ASSERT_EQ(1U, summary.ready_tokens.size());
  EXPECT_EQ(941U, summary.ready_tokens[0]);
  EXPECT_EQ(555U, summary.max_required_apply_lsn);
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverPromotionGateControlFrameAdoptsReadyEpoch) {
  Transfer_codec_context_guard codec_guard;
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid", server_uuid);
  preserve_trx_set_enable_value(true);

  const uint64_t transfer_token = 951;
  Preserved_trx_bundle bundle;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-gate-control", "source-uuid", server_uuid, bundle,
                transfer_token, &manifest, &objects));
  ...
  ASSERT_TRUE(preserved_trx_decode_promotion_adopted_epoch_marker(
      adopted_payload, &adopted_marker));
  ASSERT_EQ(1U, adopted_marker.tokens.size());
  EXPECT_EQ(transfer_token, adopted_marker.tokens[0]);
...
struct Transfer_receiver_batch_probe {
  std::mutex mutex;
  std::map<uint64_t, int> active_by_token;
  std::vector<std::pair<uint64_t, uint64_t>> applied_order;
  int active{0};
  int max_active{0};
  bool same_token_overlap{false};
};

static Preserve_trx_transfer_status transfer_receiver_batch_probe_apply(
    const Preserve_trx_transfer_frame &frame, void *context) {
  auto *probe = static_cast<Transfer_receiver_batch_probe *>(context);
  {
    std::lock_guard<std::mutex> guard(probe->mutex);
    ++probe->active;
    probe->max_active = std::max(probe->max_active, probe->active);
    const int token_active = ++probe->active_by_token[frame.token];
    if (token_active > 1) probe->same_token_overlap = true;
    probe->applied_order.emplace_back(frame.token, frame.sequence);
  }
  ...
  {
    std::lock_guard<std::mutex> guard(probe->mutex);
    --probe->active_by_token[frame.token];
    --probe->active;
  }
  return Preserve_trx_transfer_status::OK;
}

struct Transfer_receiver_promotion_barrier_probe {
  std::mutex mutex;
  bool prewarm_done{false};
};

static Preserve_trx_transfer_status
transfer_receiver_promotion_barrier_probe_apply(
    const Preserve_trx_transfer_frame &frame, void *context) {
  auto *probe =
      static_cast<Transfer_receiver_promotion_barrier_probe *>(context);
  if (frame.type == Preserve_trx_transfer_frame_type::PROMOTION_PREWARM_TOKEN) {
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    std::lock_guard<std::mutex> guard(probe->mutex);
    probe->prewarm_done = true;
    return Preserve_trx_transfer_status::OK;
  }
  if (frame.type == Preserve_trx_transfer_frame_type::PROMOTION_GATE_EPOCH) {
    std::lock_guard<std::mutex> guard(probe->mutex);
    return probe->prewarm_done ? Preserve_trx_transfer_status::OK
                               : Preserve_trx_transfer_status::CORRUPT;
  }
  return Preserve_trx_transfer_status::OK;
}
...
static Preserve_trx_transfer_status transfer_receiver_after_spool_probe(
  ...
  auto *probe = static_cast<Transfer_receiver_after_spool_probe *>(context);
  if (probe == nullptr || probe->carrier == nullptr) {
    return Preserve_trx_transfer_status::INVALID_ARGUMENT;
  }

  Preserved_trx_carrier_listing listing;
  if (probe->carrier->list_tokens(&listing) != Preserved_trx_carrier_status::OK) {
    return Preserve_trx_transfer_status::IO_ERROR;
  }
  probe->called = true;
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverFrameBatchWorkersPreserveTokenOrder) {
  std::vector<Preserve_trx_transfer_frame> frames;
  auto add_frame = [&](uint64_t token, uint64_t sequence) {
    Preserve_trx_transfer_frame frame;
    frame.type = Preserve_trx_transfer_frame_type::OBJECT_CHUNK;
    frame.epoch_id = "epoch-batch-workers";
    frame.token = token;
  ...
  EXPECT_FALSE(probe.same_token_overlap);
  EXPECT_GE(probe.max_active, 2);

  uint64_t last_sequence_for_610 = 0;
  for (const auto &entry : probe.applied_order) {
    if (entry.first != 610) continue;
    EXPECT_GT(entry.second, last_sequence_for_610);
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverBatchTreatsPromotionGateAsBarrier) {
  std::vector<Preserve_trx_transfer_frame> frames;
  Preserve_trx_transfer_frame prewarm;
  prewarm.type = Preserve_trx_transfer_frame_type::PROMOTION_PREWARM_TOKEN;
  prewarm.epoch_id = "epoch-batch-promotion";
  prewarm.token = 610;
  prewarm.sequence = 1;
  frames.push_back(prewarm);

  Preserve_trx_transfer_frame gate;
  gate.type = Preserve_trx_transfer_frame_type::PROMOTION_GATE_EPOCH;
  gate.epoch_id = "epoch-batch-promotion";
  gate.token = 0;
  gate.sequence = 2;
  frames.push_back(gate);

  Transfer_receiver_promotion_barrier_probe probe;
  EXPECT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame_batch_with_workers(
                frames, 2, transfer_receiver_promotion_barrier_probe_apply,
                &probe));
}
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 402;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_bundle_frames(
                "epoch-single-source-declare", "source-uuid", "target-uuid",
                bundle, transfer_token, 7, &sink, &manifest));
  ASSERT_GE(sink.frames().size(), 2U);

  Preserve_trx_transfer_frame first_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(sink.frames().front(),
                                               &first_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::DECLARE_TOKEN, first_frame.type);
  EXPECT_EQ(transfer_token, first_frame.token);
  EXPECT_EQ("epoch-single-source-declare", first_frame.epoch_id);

  Preserve_trx_transfer_frame second_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(sink.frames()[1],
                                               &second_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::BEGIN, second_frame.type);
...
TEST_F(PreserveSnapshotTest,
  ...
  std::vector<Preserved_trx_bundle> bundles;
  bundles.push_back(build_bundle(621));
  bundles.push_back(build_bundle(622));
  std::vector<uint64_t> transfer_tokens;
  transfer_tokens.push_back(621);
  transfer_tokens.push_back(622);

  Capturing_transfer_frame_sink source_sink;
  std::vector<Preserve_trx_transfer_manifest> manifests;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_bundles(
                "epoch-receiver-batch", "source-uuid", "target-uuid", bundles,
                transfer_tokens, 7, &source_sink, &manifests));
...
TEST_F(PreserveSnapshotTest,
       TransferSourceEpochBundlesDeclareTokensBeforeManifestFrames) {
  Transfer_codec_context_guard codec_guard;
  auto build_bundle = [&](uint64_t transfer_token) {
    Preserve_snapshot_metadata meta = metadata();
    meta.token = test_transfer_token_string(transfer_token);
  ...
  std::vector<Preserved_trx_bundle> bundles;
  bundles.push_back(build_bundle(631));
  bundles.push_back(build_bundle(632));
  std::vector<uint64_t> transfer_tokens{631, 632};

  Capturing_transfer_frame_sink sink;
  std::vector<Preserve_trx_transfer_manifest> manifests;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_bundles(
                "epoch-source-declare-first", "source-uuid", "target-uuid",
                bundles, transfer_tokens, 7, &sink, &manifests));
  ASSERT_GE(sink.frames().size(), 4U);

  for (size_t i = 0; i < transfer_tokens.size(); ++i) {
    Preserve_trx_transfer_frame frame;
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_decode_frame(sink.frames()[i], &frame));
    EXPECT_EQ(Preserve_trx_transfer_frame_type::DECLARE_TOKEN, frame.type);
    EXPECT_EQ(transfer_tokens[i], frame.token);
    EXPECT_EQ("epoch-source-declare-first", frame.epoch_id);
  }

  Preserve_trx_transfer_frame first_payload_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(
                sink.frames()[transfer_tokens.size()], &first_payload_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::BEGIN,
...
TEST_F(PreserveSnapshotTest,
  ...
  auto build_bundle = [&](uint64_t transfer_token, Preserved_trx_bundle *bundle,
    ...
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_build_portable_objects(
                  "epoch-streaming-lifecycle", "source-uuid", "target-uuid",
                  *bundle, transfer_token, manifest, objects));
  };
  ...
  Preserve_trx_transfer_manifest first_manifest;
  std::vector<Preserve_trx_transfer_object_payload> first_objects;
  build_bundle(701, &first_bundle, &first_manifest, &first_objects);

  Preserved_trx_bundle second_bundle;
  Preserve_trx_transfer_manifest second_manifest;
  std::vector<Preserve_trx_transfer_object_payload> second_objects;
  build_bundle(702, &second_bundle, &second_manifest, &second_objects);

  Capturing_transfer_frame_sink sink;
  uint64_t next_sequence = 1;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_begin_frames(
                {first_manifest, second_manifest}, &sink, &next_sequence));
  ASSERT_EQ(2U, sink.frames().size());
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_object_frames(
                first_manifest, first_objects, 7, &sink, &next_sequence));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_object_frames(
                second_manifest, second_objects, 7, &sink, &next_sequence));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_commit_frame(
                first_manifest.epoch_id, first_manifest.token, &sink,
                &next_sequence));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, apply_new_frames());
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 703;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-session-external-objects", "source-uuid",
                "target-uuid", bundle, transfer_token, &manifest, &objects));
  ASSERT_FALSE(objects.empty());

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-external-objects", "source-uuid", "target-uuid", 7,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  ASSERT_GE(source_sink.frames().size(), 4U);
  Preserve_trx_transfer_frame first_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(source_sink.frames()[0],
                                               &first_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::DECLARE_TOKEN, first_frame.type);

  Preserve_trx_transfer_frame second_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(source_sink.frames()[1],
                                               &second_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::BEGIN, second_frame.type);
  EXPECT_EQ(transfer_token, second_frame.token);

  bool saw_object_chunk = false;
  bool saw_seal = false;
  bool saw_commit = false;
  for (const std::string &encoded_frame : source_sink.frames()) {
    Preserve_trx_transfer_frame frame;
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_decode_frame(encoded_frame, &frame));
    if (frame.type == Preserve_trx_transfer_frame_type::OBJECT_CHUNK) {
      saw_object_chunk = true;
    } else if (frame.type == Preserve_trx_transfer_frame_type::SEAL_OBJECT) {
      saw_seal = true;
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 704;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-session-stream-object", "source-uuid", "target-uuid",
                bundle, transfer_token, &manifest, &objects));
  ASSERT_EQ(1U, objects.size());
  ASSERT_GT(objects.front().payload.length(), 3U);

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-stream-object", "source-uuid", "target-uuid", 7,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.begin_token_objects(manifest));

  const std::string object_id = objects.front().descriptor.object_id;
  const std::string &payload = objects.front().payload;
  const size_t split = payload.length() / 2;
  ASSERT_GT(split, 0U);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(transfer_token, object_id, 0,
                                       payload.substr(0, split)));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(transfer_token, object_id, split,
                                       payload.substr(split)));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.seal_object(transfer_token, object_id));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.finalize_token_manifest(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  ...
  for (const std::string &encoded_frame : source_sink.frames()) {
    ...
    if (frame.type == Preserve_trx_transfer_frame_type::BEGIN) {
      saw_begin = true;
    } else if (frame.type == Preserve_trx_transfer_frame_type::OBJECT_CHUNK) {
      ++chunk_count;
    } else if (frame.type == Preserve_trx_transfer_frame_type::SEAL_OBJECT) {
      saw_seal = true;
    } else if (frame.type == Preserve_trx_transfer_frame_type::COMMIT_EPOCH) {
...
TEST_F(PreserveSnapshotTest,
  ...
  Preserve_snapshot_metadata meta = logged_with_cache_metadata();
  meta.token = test_transfer_token_string(transfer_token);
  const std::string blob_payload = "phase1-streamed-external-blob";

  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  Mysql_binlog_preserve_snapshot logged_binlog;
  logged_binlog.cache_payload = blob_payload;
  logged_binlog.gtid_next = "AUTOMATIC";
  logged_binlog.event_counter = 3;
  logged_binlog.with_rbr = true;
  logged_binlog.with_start = true;
  logged_binlog.with_content = true;
  input.logged_binlog_snapshot = &logged_binlog;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-session-declare-object", "source-uuid", "target-uuid",
                bundle, transfer_token, &manifest, &objects));
  ASSERT_EQ(2U, objects.size());

  const Preserve_trx_transfer_object_payload *blob_object = nullptr;
  const Preserve_trx_transfer_object_payload *snapshot_object = nullptr;
  for (const Preserve_trx_transfer_object_payload &object : objects) {
    if (object.descriptor.kind ==
        Preserve_trx_transfer_object_kind::EXTERNAL_BLOB) {
      blob_object = &object;
    } else if (object.descriptor.kind ==
               Preserve_trx_transfer_object_kind::SNAPSHOT_BUNDLE) {
      snapshot_object = &object;
    }
  }
  ASSERT_NE(nullptr, blob_object);
  ASSERT_NE(nullptr, snapshot_object);

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-declare-object", "source-uuid", "target-uuid", 7,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_object(transfer_token, blob_object->descriptor));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(transfer_token,
                                       blob_object->descriptor.object_id, 0,
                                       blob_object->payload));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.seal_object(transfer_token,
                                blob_object->descriptor.object_id));

  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.begin_token_objects(manifest));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(transfer_token,
                                       snapshot_object->descriptor.object_id, 0,
                                       snapshot_object->payload));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.seal_object(transfer_token,
                                snapshot_object->descriptor.object_id));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.finalize_token_manifest(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());

  ASSERT_GE(source_sink.frames().size(), 6U);
  Preserve_trx_transfer_frame declare_object_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(source_sink.frames()[1],
                                               &declare_object_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::DECLARE_OBJECT,
            declare_object_frame.type);
  Preserve_trx_transfer_frame begin_frame;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(source_sink.frames()[4],
                                               &begin_frame));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::BEGIN, begin_frame.type);
...
TEST_F(PreserveSnapshotTest,
  ...
  Preserve_snapshot_metadata meta = logged_with_cache_metadata();
  meta.token = test_transfer_token_string(transfer_token);
  const std::string blob_payload = "phase1-object-prewarm-external-blob";
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-phase1-object-prewarm", "source-uuid", "target-uuid",
                bundle, transfer_token, &manifest, &objects));
  ...
  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-phase1-object-prewarm", "source-uuid", "target-uuid", 7,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.seal_object(transfer_token,
                                blob_object->descriptor.object_id));
  ASSERT_EQ(4U, source_sink.frames().size());
  ...
  Preserved_trx_store store(&carrier);
  Preserve_trx_transfer_receiver_registry registry;
  const uint64_t proof_count_before =
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 718;
  const std::string record_payload = record_locks_payload();

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "phase1-prewarm-record", record_payload, &prebuilt_record);

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-phase1-prewarm-manifest", "source-uuid", "target-uuid", 17,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_stream_prebuilt_record_locks_blob(
                &session, transfer_token, m_dir, prebuilt_record));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  Preserve_trx_transfer_receiver_record record;
  ASSERT_TRUE(registry.lookup("epoch-phase1-prewarm-manifest", transfer_token,
                              &record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::RECEIVING, record.state);
  ASSERT_EQ(1U, record.objects.size());
  EXPECT_EQ(kPreservedTrxBlobRecordLocks, record.objects[0].object_id);
  EXPECT_TRUE(registry.all_objects_sealed(record.epoch_id, record.token));
  for (uint attempt = 0; attempt < 100; ++attempt) {
  ...
  EXPECT_EQ(record_object_prewarm_before + 1,
            preserve_trx_transfer_receiver_record_object_prewarm_count_status());
}
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink phase1_sink;
  Preserve_trx_transfer_source_epoch_session phase1_session(
      "epoch-prewarm-priority", "source-uuid", "target-uuid", 17,
      &phase1_sink);
  for (uint64_t index = 0; index < 4; ++index) {
    const uint64_t token = 8300 + index;
    Local_file_preserved_trx_carrier warm_carrier(m_dir);
    PrebuiltRecordLocksBlob prebuilt_record;
    create_warm_prebuilt_record_locks_blob(
        &warm_carrier, "priority-record-" + std::to_string(index),
        record_locks_payload(), &prebuilt_record);
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              phase1_session.declare_token(token));
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_stream_prebuilt_record_locks_blob(
                  &phase1_session, token, m_dir, prebuilt_record));
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              phase1_session.begin_token_prewarm_manifest(token));
  }
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_handle_receiver_payload_batch(
                m_dir, phase1_sink.frames(), &store, &registry, 300, 2));

  my_sleep(20000);

  const uint64_t staged_token_total_before =
      preserve_trx_transfer_receiver_staged_token_total_us_status();
  const uint64_t staged_token = 8400;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(staged_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  ASSERT_EQ(Preserve_snapshot_status::OK,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-staged-priority", "source-uuid", "target-uuid",
                bundle, staged_token, &manifest, &objects));
  std::string manifest_payload;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(manifest, &manifest_payload));
  ...
  bool staged_token_prewarmed = false;
  for (uint attempt = 0; attempt < 200; ++attempt) {
    if (preserve_trx_transfer_receiver_staged_token_total_us_status() >
        staged_token_total_before) {
      staged_token_prewarmed = true;
      break;
    }
    my_sleep(10000);
  }
  ASSERT_TRUE(staged_token_prewarmed);
  EXPECT_LT(preserve_trx_transfer_receiver_record_object_prewarm_count_status() -
                record_object_prewarm_before,
            4U);
}
...
TEST_F(PreserveSnapshotTest,
  ...
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "phase1-prewarm-record-dedup", record_payload,
      &prebuilt_record);
  ...
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  input.prebuilt_record_locks_blob = &prebuilt_record;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-phase1-prewarm-dedup", "source-uuid", "target-uuid", 17,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  size_t consumed_frames = 0;
  auto apply_new_frames = [&]() {
    std::vector<std::string> frames(source_sink.frames().begin() +
                                        consumed_frames,
                                    source_sink.frames().end());
    consumed_frames = source_sink.frames().size();
    return preserve_trx_transfer_handle_receiver_payload_batch(
        m_dir, frames, &store, &registry, 300, 3);
  };
  ...
  phase1_manifest.source_server_uuid = phase1_record.source_server_uuid;
  phase1_manifest.target_server_uuid = phase1_record.target_server_uuid;
  ...
  bool durable_snapshot_may_exist = false;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            artifact_sink.publish_bundle(std::move(bundle), 300,
                                         &written_metadata,
                                         &durable_snapshot_may_exist));
  EXPECT_TRUE(durable_snapshot_may_exist);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, apply_new_frames());
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 716;
  const std::string malformed_payload("\1\2\3", 3);

  Preserve_trx_transfer_object_descriptor descriptor;
  descriptor.object_id = kPreservedTrxBlobRecordLocks;
  descriptor.kind = Preserve_trx_transfer_object_kind::EXTERNAL_BLOB;
  descriptor.total_size = malformed_payload.length();
  descriptor.digest = test_sha256(malformed_payload);

  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-record-object-malformed", "source-uuid", "target-uuid", 7,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.write_object_chunk(transfer_token, descriptor.object_id, 0,
                                       malformed_payload));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  const uint64_t proof_count_before =
      preserve_trx_transfer_receiver_object_prewarm_proof_count_status();
  const uint64_t miss_count_before =
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink source_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-no-cold-fallback", "source-uuid", "target-uuid", 11,
      &source_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  Preserve_trx_transfer_manifest manifest;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(bundle, transfer_token, &manifest));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  ...
  Preserved_trx_store store(&carrier);
  Preserve_trx_transfer_receiver_registry registry;
  const uint64_t fallback_count_before =
...
TEST_F(PreserveSnapshotTest,
  ...
  constexpr uint64_t kTokenCount = 32;
  for (uint64_t token_index = 0; token_index < kTokenCount; ++token_index) {
    const uint64_t token = 9000 + token_index;
    Preserve_trx_transfer_manifest manifest;
    ...
    manifest.source_epoch_commit_lsn = 200 + token_index;

    Preserve_trx_transfer_object_descriptor snapshot;
    snapshot.object_id = "snapshot";
    snapshot.kind = Preserve_trx_transfer_object_kind::SNAPSHOT_BUNDLE;
    snapshot.total_size = 1;
    snapshot.digest.fill(static_cast<unsigned char>(token_index + 1));
    manifest.objects.push_back(snapshot);
    ...
    const std::string token_component = test_transfer_token_string(token);
    carrier.snapshots[token_component] = {'x'};
    carrier.standby_pending_tokens.insert(token_component);
  }

  carrier.list_token_reads = 0;
  Preserve_trx_transfer_frame commit;
  commit.type = Preserve_trx_transfer_frame_type::COMMIT_EPOCH;
  commit.sequence = 1;
  commit.epoch_id = "epoch-commit-list-once";
  commit.token = 9000;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, commit, &store, &registry, 300, nullptr));

  EXPECT_LE(carrier.list_token_reads, 2U);
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverTokenDeclareWaitsForManifestBeforeCommit) {
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid", "target-uuid");
  ...
  declare.type = Preserve_trx_transfer_frame_type::DECLARE_TOKEN;
  declare.sequence = 1;
  declare.epoch_id = "epoch-declare-token";
  declare.token = 901;
  declare.reason = "source-uuid\ntarget-uuid";
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
  ...
  Preserve_trx_transfer_receiver_record record;
  ASSERT_TRUE(registry.lookup("epoch-declare-token", 901, &record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::DECLARED, record.state);
  EXPECT_EQ("source-uuid", record.source_server_uuid);
  EXPECT_EQ("target-uuid", record.target_server_uuid);

  Preserved_trx_carrier_token_state token_state;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(test_transfer_token_string(901),
                                &token_state));
  EXPECT_FALSE(token_state.snapshot);
  EXPECT_FALSE(token_state.standby_pending);
  ...
  commit.type = Preserve_trx_transfer_frame_type::COMMIT_EPOCH;
  commit.sequence = 2;
  commit.epoch_id = "epoch-declare-token";
  commit.token = 901;
  EXPECT_EQ(Preserve_trx_transfer_status::CORRUPT,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, commit, &store, &registry, 300, nullptr));
...
TEST_F(PreserveSnapshotTest,
       TransferReceiverRejectsConfiguredTargetThatIsNotLocalServerUuid) {
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid", "target-uuid");
  std::snprintf(server_uuid, UUID_LENGTH + 1, "%s",
                "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee");
...
TEST_F(PreserveSnapshotTest, TransferSourceCanDeclareTokenBeforeManifest) {
  ...
  uint64_t next_sequence = 1;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_declare_token_frame(
                "epoch-source-declare", "source-uuid", "target-uuid", 902,
                &sink, &next_sequence));
  ASSERT_EQ(1U, sink.frames().size());
  ...
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(902);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  ASSERT_EQ(Preserve_snapshot_status::OK,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-source-declare", "source-uuid", "target-uuid", bundle,
                902, &manifest, &objects));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_begin_frames(
                {manifest}, &sink, &next_sequence));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_object_frames(
                manifest, objects, 7, &sink, &next_sequence));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_send_epoch_commit_frame(
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-source-session-abort", "source-uuid", "target-uuid", 7, &sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(904));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(905));

  Preserve_trx_transfer_manifest manifest;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(build_bundle(904), 904, &manifest));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.abort_token(905, "source_phase1_target_removed"));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  ...
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(test_transfer_token_string(905),
                                &removed_state));
  EXPECT_FALSE(removed_state.snapshot);
  EXPECT_FALSE(removed_state.standby_pending);

  Preserve_trx_transfer_receiver_record removed_record;
  ASSERT_TRUE(registry.lookup(manifest.epoch_id, 905, &removed_record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::ABORTED,
            removed_record.state);
  EXPECT_EQ("source_phase1_target_removed", removed_record.last_error);
  EXPECT_TRUE(preserve_trx_transfer_epoch_committed(m_dir, manifest));
}
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-source-session-batch-final", "source-uuid", "target-uuid", 11,
      &sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(912));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(913));
  ASSERT_EQ(2U, sink.frames().size());

  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(build_bundle(912), 912));
  EXPECT_EQ(2U, sink.frames().size());
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(build_bundle(913), 913));
  EXPECT_EQ(2U, sink.frames().size());
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-source-session-spool-signal", "source-uuid", "target-uuid", 11,
      &sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(914));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(915));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(build_bundle(914), 914));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(build_bundle(915), 915));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_handle_receiver_payload_batch(
                m_dir, sink.frames(), &store, &registry, 300, 3, nullptr,
                transfer_receiver_after_spool_probe, &probe));
...
TEST_F(PreserveSnapshotTest,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-seal-prepublish", "source-uuid", "target-uuid", bundle,
                916, &manifest, &objects));
  ...
  declare.epoch_id = manifest.epoch_id;
  declare.token = manifest.token;
  declare.reason = manifest.source_server_uuid + "\n" +
                   manifest.target_server_uuid;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, declare, &store, &registry, 300, nullptr));
  ...
  Preserved_trx_carrier_token_state state;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(test_transfer_token_string(manifest.token),
                                &state));
  EXPECT_FALSE(state.snapshot);
  EXPECT_FALSE(state.standby_pending);
}
...
TEST_F(PreserveSnapshotTest,
  ...
  const uint64_t transfer_token = 927;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-sink-prepublish", "source-uuid", "target-uuid", 11,
      &frame_sink);

  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));

  Preserve_trx_transfer_session_artifact_sink sink(&session, transfer_token,
                                                   m_dir);
  Preserve_snapshot_metadata written_metadata;
  bool durable_snapshot_may_exist = false;
  ASSERT_EQ(Preserve_snapshot_status::OK,
  ...
  Preserved_trx_carrier_token_state state;
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            carrier.token_state(test_transfer_token_string(transfer_token),
                                &state));
  EXPECT_FALSE(state.snapshot);
  EXPECT_FALSE(state.standby_pending);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  EXPECT_TRUE(preserve_trx_transfer_epoch_committed(
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 928;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-sink-batch-final", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ...
  EXPECT_TRUE(durable_snapshot_may_exist);
  EXPECT_EQ(meta.token, written_metadata.token);
  EXPECT_EQ(2U, frame_sink.frames().size());
}

TEST_F(PreserveSnapshotTest,
       TransferReceiverCommitEpochDoesNotSynchronouslyPublishToken) {
  Transfer_codec_context_guard codec_guard;
  Transfer_receiver_config_guard receiver_config;
  receiver_config.allow("source-uuid", "target-uuid");

  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(917);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  ASSERT_EQ(Preserve_snapshot_status::OK,
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-commit-no-sync-publish", "source-uuid", "target-uuid",
                bundle, 917, &manifest, &objects));

  Local_file_preserved_trx_carrier carrier(m_dir);
  Preserved_trx_store store(&carrier);
  Preserve_trx_transfer_receiver_registry registry;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            registry.begin_receive(manifest, 1));
  for (const Preserve_trx_transfer_object_payload &object : objects) {
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_stage_object_chunk(
                  m_dir, manifest, object.descriptor.object_id, 0,
...
TEST_F(PreserveSnapshotTest,
       TransferSourceEpochSessionRejectsCommitWithUnresolvedDeclaredToken) {
  Transfer_codec_context_guard codec_guard;

  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(906);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  ASSERT_EQ(Preserve_snapshot_status::OK,
  ...
  Capturing_transfer_frame_sink sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-source-session-unresolved", "source-uuid", "target-uuid", 7,
      &sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(906));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(907));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(bundle, 906, nullptr));

  const size_t frame_count_before_commit = sink.frames().size();
  EXPECT_EQ(Preserve_trx_transfer_status::UNSUPPORTED,
            session.commit_epoch());
  ASSERT_EQ(frame_count_before_commit, sink.frames().size());
  for (const std::string &encoded_frame : sink.frames()) {
    Preserve_trx_transfer_frame frame;
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              preserve_trx_transfer_decode_frame(encoded_frame, &frame));
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 nullptr);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-abort", "source-uuid", "target-uuid", 11, &frame_sink);

  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(908));
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.declare_token(909));
  Preserved_trx_bundle first = build_bundle(908);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.send_token_bundle(first, 908));

  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.abort_epoch("source_batch_failed"));
  EXPECT_EQ(Preserve_trx_transfer_status::UNSUPPORTED, session.commit_epoch());

  Preserve_trx_transfer_receiver_record first_record;
  ASSERT_TRUE(registry.lookup("epoch-session-abort", 908, &first_record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::ABORTED, first_record.state);
  EXPECT_EQ("source_batch_failed", first_record.last_error);

  Preserve_trx_transfer_receiver_record second_record;
  ASSERT_TRUE(registry.lookup("epoch-session-abort", 909, &second_record));
...
TEST_F(PreserveSnapshotTest, TransferReceiverAbortCanCancelDeclaredToken) {
  ...
  declare.type = Preserve_trx_transfer_frame_type::DECLARE_TOKEN;
  declare.sequence = 1;
  declare.epoch_id = "epoch-declare-abort";
  declare.token = 903;
  declare.reason = "source-uuid\ntarget-uuid";
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
  ...
  abort.type = Preserve_trx_transfer_frame_type::ABORT;
  abort.sequence = 2;
  abort.epoch_id = "epoch-declare-abort";
  abort.token = 903;
  abort.reason = "source_phase1_abort";
  EXPECT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_apply_receiver_frame(
                m_dir, abort, &store, &registry, 300, nullptr));

  Preserve_trx_transfer_receiver_record record;
  ASSERT_TRUE(registry.lookup("epoch-declare-abort", 903, &record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::ABORTED, record.state);
  EXPECT_EQ("source_phase1_abort", record.last_error);
}

TEST_F(PreserveSnapshotTest, TransferReceiverCommitEpochReadiesEverySealedToken) {
  Transfer_codec_context_guard codec_guard;
  auto build_bundle = [&](uint64_t transfer_token, Preserved_trx_bundle *bundle,
                          Preserve_trx_transfer_manifest *manifest,
                          std::vector<Preserve_trx_transfer_object_payload>
  ...
  Preserve_trx_promotion_ready_summary ready_summary;
  ASSERT_TRUE(wait_for_epoch_ready(first_manifest.epoch_id, 2, &ready_summary));
  std::set<uint64_t> ready_tokens(ready_summary.ready_tokens.begin(),
                                  ready_summary.ready_tokens.end());
  EXPECT_EQ((std::set<uint64_t>{first_manifest.token, second_manifest.token}),
            ready_tokens);
  Preserve_trx_transfer_epoch_fact fact;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_read_epoch_fact(
                m_dir, first_manifest.epoch_id, &fact));
  EXPECT_EQ(first_manifest.epoch_id, fact.epoch_id);
  EXPECT_EQ(first_manifest.source_server_uuid, fact.source_server_uuid);
  EXPECT_EQ(first_manifest.target_server_uuid, fact.target_server_uuid);
  ASSERT_EQ(2U, fact.tokens.size());
  EXPECT_EQ(first_manifest.token, fact.tokens[0].token);
  EXPECT_EQ(second_manifest.token, fact.tokens[1].token);
  std::string first_encoded_manifest;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(first_manifest,
                                                  &first_encoded_manifest));
  EXPECT_EQ(test_sha256(first_encoded_manifest),
            fact.tokens[0].manifest_digest);
  std::string second_encoded_manifest;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_encode_manifest(second_manifest,
                                                  &second_encoded_manifest));
  EXPECT_EQ(test_sha256(second_encoded_manifest),
            fact.tokens[1].manifest_digest);
}
...
TEST_F(PreserveSnapshotTest, TransferSourceUsesSingleSequencedConnection) {
  Transfer_codec_context_guard codec_guard;
  Mysql_binlog_preserve_snapshot snapshot;
  snapshot.cache_payload.assign(128, 'x');
  snapshot.gtid_next = "AUTOMATIC";
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 604;
  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-artifact", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));

  const uint64_t bulk_bytes_before =
      preserve_trx_transfer_phase2_bulk_bytes_status();
  const uint64_t final_metadata_fsyncs_before =
  ...
  EXPECT_TRUE(durable_snapshot_may_exist);
  EXPECT_EQ(meta.token, written_metadata.token);
  EXPECT_EQ("", receiver_metadata.token);

  Preserved_trx_carrier_token_state state;
  if (carrier.token_state(meta.token, &state) ==
      Preserved_trx_carrier_status::OK) {
    EXPECT_FALSE(state.standby_pending);
  }
  EXPECT_FALSE(preserve_trx_transfer_epoch_committed(
      m_dir, std::string("epoch-session-artifact")));
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 606;
  Mysql_binlog_preserve_snapshot snapshot;
  snapshot.cache_payload = "phase1-presealed-binlog-cache-payload";
  snapshot.gtid_next = "AUTOMATIC";
  snapshot.event_counter = 12;
  snapshot.with_rbr = true;
  snapshot.with_start = true;
  snapshot.with_content = true;
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-presealed", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ...
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                ...
                session.target_server_uuid(), bundle, transfer_token, &manifest,
                &objects));
  for (const Preserve_trx_transfer_object_descriptor &descriptor :
    ...
    const auto payload_it = std::find_if(
        ...
        [&](const Preserve_trx_transfer_object_payload &payload) {
          return payload.descriptor.object_id == descriptor.object_id;
        });
    ASSERT_NE(objects.end(), payload_it);
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              session.declare_object(transfer_token, descriptor));
    for (uint64_t offset = 0; offset < payload_it->payload.length();
         offset += session.chunk_bytes()) {
      const size_t length = std::min<uint64_t>(
          session.chunk_bytes(), payload_it->payload.length() - offset);
      ASSERT_EQ(Preserve_trx_transfer_status::OK,
                session.write_object_chunk(
                    transfer_token, descriptor.object_id, offset,
                    payload_it->payload.substr(offset, length)));
    }
    ASSERT_EQ(Preserve_trx_transfer_status::OK,
              session.seal_object(transfer_token, descriptor.object_id));
  }
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 604;
  const std::string record_payload =
      record_locks_payload() + "streamed-prebuilt-record-locks";

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "streamed-record-warmcopy", record_payload,
      &prebuilt_record);
  ...
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-presealed-prebuilt", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
  ...
  /*
    The source session already owns the sealed portable object. Removing the
    warmcopy scratch blob proves publish_bundle() does not read it again during
    phase 2 while it writes the final snapshot and manifest metadata.
  */
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            warm_carrier.remove_warm_external_blob(
                prebuilt_record.warmcopy_id, prebuilt_record.name));
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 616;
  const std::string stale_record_payload =
      record_locks_payload() + "stale-phase1-presealed-record-locks";
  const std::string final_record_payload = record_locks_payload();

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob stale_prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "stale-record-warmcopy", stale_record_payload,
      &stale_prebuilt_record);

  Preserve_snapshot_metadata meta = metadata();
  meta.token = test_transfer_token_string(transfer_token);
  meta.record_locks_payload = final_record_payload;
  meta.mdl_descriptors_payload = empty_mdl_descriptors_payload();
  Preserved_trx_bundle bundle;
  Preserved_trx_bundle_build_input input;
  input.metadata = meta;
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  ASSERT_EQ(1U, bundle.external_blobs.size());
  ASSERT_FALSE(bundle.external_blobs[0].prebuilt);
  ASSERT_EQ(kPreservedTrxBlobRecordLocks, bundle.external_blobs[0].name);
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-stale-presealed-record", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_stream_prebuilt_record_locks_blob(
                &session, transfer_token, m_dir, stale_prebuilt_record));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.begin_token_prewarm_manifest(transfer_token));
  ...
  EXPECT_TRUE(durable_snapshot_may_exist);
  EXPECT_EQ(meta.token, written_metadata.token);
  EXPECT_GT(preserve_trx_transfer_phase2_bulk_bytes_status(),
            bulk_bytes_before);
  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 617;
  const std::string stale_record_payload =
      record_locks_payload() + "stale-phase1-record-catchup";
  const std::string final_record_payload =
      record_locks_payload() + "final-quiesced-record-catchup";
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-record-catchup-replace", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ...
  PrebuiltRecordLocksBlob stale_prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "stale-record-catchup", stale_record_payload,
      &stale_prebuilt_record);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_stream_prebuilt_record_locks_blob(
                &session, transfer_token, m_dir, stale_prebuilt_record));
  ...
  PrebuiltRecordLocksBlob final_prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "final-record-catchup", final_record_payload,
      &final_prebuilt_record);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_stream_prebuilt_record_locks_blob(
                &session, transfer_token, m_dir, final_prebuilt_record));

  Preserve_trx_transfer_object_descriptor final_descriptor;
  final_descriptor.object_id = kPreservedTrxBlobRecordLocks;
  final_descriptor.kind = Preserve_trx_transfer_object_kind::EXTERNAL_BLOB;
  final_descriptor.total_size = final_prebuilt_record.size;
  final_descriptor.digest = final_prebuilt_record.digest;
  EXPECT_TRUE(session.object_presealed_for_token(transfer_token,
                                                final_descriptor));

  Preserve_trx_transfer_receiver_record record;
  ASSERT_TRUE(registry.lookup("epoch-session-record-catchup-replace",
                              transfer_token, &record));
  EXPECT_EQ(Preserve_trx_transfer_receiver_state::RECEIVING, record.state);
  ASSERT_EQ(1U, record.objects.size());
  EXPECT_EQ(final_descriptor.object_id, record.objects[0].object_id);
  EXPECT_EQ(final_descriptor.kind, record.objects[0].kind);
  EXPECT_EQ(final_descriptor.total_size, record.objects[0].total_size);
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 614;
  const std::string record_payload =
      record_locks_payload() + "unsealed-prebuilt-record-locks";

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "unsealed-record-warmcopy", record_payload,
      &prebuilt_record);
  ...
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-unsealed-prebuilt", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
...
TEST_F(PreserveSnapshotTest,
       TransferSessionArtifactSinkUsesPresealedPrebuiltBinlogCacheBlob) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 615;
  const std::string binlog_payload =
      "streamed-prebuilt-binlog-cache-payload";

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltBinlogCacheBlob prebuilt_binlog;
  create_warm_prebuilt_binlog_blob(
      &warm_carrier, "streamed-binlog-warmcopy", binlog_payload,
      &prebuilt_binlog);
  ...
  input.metadata = meta;
  input.metadata.mdl_descriptors_payload = empty_mdl_descriptors_payload();
  input.prebuilt_binlog_cache_blob = &prebuilt_binlog;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  ASSERT_EQ(1U, bundle.external_blobs.size());
  ASSERT_TRUE(bundle.external_blobs[0].prebuilt);
  ...
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-session-presealed-binlog", "source-uuid", "target-uuid", 11,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_stream_prebuilt_binlog_cache_blob(
                &session, transfer_token, m_dir, prebuilt_binlog));

  /*
    Phase 2 should only publish final metadata. Removing the source warm blob
    proves the transfer sink consumes the already-sealed receiver object instead
    of reading and sending binlog cache bytes during snapshot publication.
  */
  ASSERT_EQ(Preserved_trx_carrier_status::OK,
            warm_carrier.remove_warm_external_blob(
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 607;
  const std::string record_payload = record_locks_payload();

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "receiver-record-warmcopy", record_payload,
      &prebuilt_record);
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-receiver-presealed-prebuilt", "source-uuid", server_uuid, 11,
      &frame_sink);
  ...
  for (uint attempt = 0; attempt < 100; ++attempt) {
    if (preserve_trx_promotion_prewarm_record_lock_page_count_status() > 0) {
      break;
    }
    my_sleep(10000);
  }
  ...
  const uint64_t resident_pages =
      preserve_trx_promotion_prewarm_record_lock_resident_pages_status();
  EXPECT_GT(page_count, 0U);
  EXPECT_LE(resident_pages, page_count);
  EXPECT_EQ(page_count - resident_pages,
            preserve_trx_promotion_prewarm_record_lock_cold_page_gets_status());
}
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 608;
  const std::string record_payload =
      record_locks_payload() + "receiver-presealed-record-locks-with-binlog";
  Mysql_binlog_preserve_snapshot binlog_snapshot;
  binlog_snapshot.cache_payload = "receiver-binlog-cache-external-payload";
  binlog_snapshot.gtid_next = "AUTOMATIC";
  binlog_snapshot.event_counter = 21;
  binlog_snapshot.with_rbr = true;
  binlog_snapshot.with_start = true;
  binlog_snapshot.with_content = true;
  ...
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "receiver-record-warmcopy-binlog", record_payload,
      &prebuilt_record);
  ...
  PrebuiltBinlogCacheBlob prebuilt_binlog;
  create_warm_prebuilt_binlog_blob(
      &warm_carrier, "phase1-prewarm-binlog",
      binlog_snapshot.cache_payload, &prebuilt_binlog);
  input.prebuilt_binlog_cache_blob = &prebuilt_binlog;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            build_preserved_trx_bundle(input, &bundle));
  ASSERT_EQ(2U, bundle.external_blobs.size());
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-receiver-presealed-record-binlog", "source-uuid", "target-uuid",
      11, &frame_sink);
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-artifact-predeclare", "source-uuid", "target-uuid", 9,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  Preserve_trx_transfer_session_artifact_sink sink(&session, transfer_token,
  ...
  ASSERT_EQ(3U, frame_sink.frames().size());
  Preserve_trx_transfer_frame declare_token;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(frame_sink.frames().front(),
                                               &declare_token));
  EXPECT_EQ(Preserve_trx_transfer_frame_type::DECLARE_TOKEN, declare_token.type);

  Preserve_trx_transfer_frame commit;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_decode_frame(frame_sink.frames().back(),
                                               &commit));
...
TEST_F(PreserveSnapshotTest,
  ...
  const ulonglong saved_max_binlog_cache_size = max_binlog_cache_size;
  const ulong saved_binlog_stmt_cache_size = binlog_stmt_cache_size;
  const ulonglong saved_max_binlog_stmt_cache_size = max_binlog_stmt_cache_size;
  handlerton binlog_hton_for_test{};
  binlog_hton_for_test.slot = 0;
  binlog_hton_for_test.savepoint_offset = sizeof(my_off_t);
  preserve_trx_set_enable_value(true);
  opt_bin_log = true;
  binlog_cache_size = 32 * 1024;
  max_binlog_cache_size = 4 * 1024 * 1024;
  binlog_stmt_cache_size = 32 * 1024;
  max_binlog_stmt_cache_size = 4 * 1024 * 1024;
  mysql_binlog_preserve_set_runtime_for_unit_test(&binlog_hton_for_test, true);
  preserve_trx_resource_manager_reset_for_unit_test();
  preserve_trx_resource_manager_set_limits_for_unit_test(
      {16 * 1024 * 1024, 16 * 1024 * 1024});
  preserve_trx_resource_manager_set_external_limits_for_unit_test(
      {4096, 16, 16ULL * 1024ULL * 1024ULL * 1024ULL, true});
  auto restore_runtime = create_scope_guard([&] {
    preserve_trx_transfer_shutdown_receiver_prewarm_workers();
    preserve_trx_resource_manager_reset_for_unit_test();
    preserve_trx_resource_manager_set_limits_for_unit_test(
        Preserve_trx_resource_limits{});
    preserve_trx_resource_manager_set_external_limits_for_unit_test({});
    mysql_binlog_preserve_set_runtime_for_unit_test(nullptr, false);
    max_binlog_stmt_cache_size = saved_max_binlog_stmt_cache_size;
    binlog_stmt_cache_size = saved_binlog_stmt_cache_size;
    max_binlog_cache_size = saved_max_binlog_cache_size;
    binlog_cache_size = saved_binlog_cache_size;
    opt_bin_log = saved_opt_bin_log;
    preserve_trx_set_enable_value(saved_enable);
  });

  const uint64_t transfer_token = 609;
  Mysql_binlog_preserve_snapshot snapshot;
  snapshot.cache_payload.assign(128 * 1024, 'n');
  snapshot.gtid_next = "AUTOMATIC";
  snapshot.event_counter = 11;
  snapshot.with_rbr = true;
  snapshot.with_start = true;
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                  nullptr);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-native-binlog-ready", "source-uuid", server_uuid, 11,
      &frame_sink);
  Preserve_trx_transfer_manifest manifest;
  std::vector<Preserve_trx_transfer_object_payload> objects;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_build_portable_objects(
                "epoch-native-binlog-ready", "source-uuid", server_uuid,
                bundle, transfer_token, &manifest, &objects));
  const Preserve_trx_transfer_object_descriptor *record_object = nullptr;
  ...
  ASSERT_NE(nullptr, record_object);
  lock_preserve_record_lock_metadata_facts_t record_facts;
  ASSERT_EQ(lock_preserve_metadata_plan_status::OK,
            lock_preserve_build_record_lock_metadata_facts(
                meta.record_locks_payload, &record_facts));
  lock_preserve_metadata_plan_validation_t validation;
  validation.object_generation = manifest.source_epoch_commit_lsn;
  validation.expected_object_generation = manifest.source_epoch_commit_lsn;
  validation.physical_fence_lsn = manifest.source_epoch_commit_lsn;
  ...
  for (const unsigned char byte : record_object->digest) {
    validation.object_digest.push_back(kHex[(byte >> 4) & 0x0f]);
    validation.object_digest.push_back(kHex[byte & 0x0f]);
  }
  validation.final_lock_generation_digest =
      record_facts.final_lock_generation_digest;
  validation.page_layout_digest = record_facts.page_layout_digest;
  validation.dictionary_generation_digest =
      record_facts.dictionary_generation_digest;
  validation.implicit_native_continuity_proven = true;
  validation.is_final_quiesced = true;
  ...
  auto record_plan = std::make_unique<lock_preserve_metadata_plan_t>();
  ASSERT_EQ(lock_preserve_metadata_plan_status::OK,
            lock_preserve_build_record_lock_metadata_plan(
                meta.record_locks_payload, validation, lease_ops,
                record_plan.get()));
  ASSERT_TRUE(preserve_trx_transfer_put_receiver_record_lock_plan_for_unit_test(
      m_dir, manifest, std::move(record_plan), record_facts));
  Preserve_trx_prepared_token_key strict_key;
  ASSERT_TRUE(preserve_trx_transfer_strict_prepared_key_for_unit_test(
      m_dir, manifest, bundle.metadata.token, &strict_key));
  auto &strict_registry = preserved_trx_strict_prepared_token_registry();
  ...
  EXPECT_EQ(Preserve_trx_prepared_token_state::READY_FOR_GATE,
            strict_snapshot.state);
  EXPECT_TRUE(strict_snapshot.record_lock_plan_owned);
  EXPECT_TRUE(strict_snapshot.native_binlog_handle_owned);
  EXPECT_EQ(record_facts.bitmap_entries,
            strict_snapshot.facts.record_lock_bitmap_entries);
  EXPECT_EQ(record_facts.bitmap_bits,
            strict_snapshot.facts.record_lock_bits);
  EXPECT_TRUE(strict_snapshot.facts.binlog_cache_present);
  EXPECT_EQ(snapshot.cache_payload.size(),
...
TEST_F(PreserveSnapshotTest,
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_artifact_sink sink(
      "epoch-artifact-deadline", "source-uuid", "target-uuid",
      transfer_token, 11, &frame_sink);

  Preserve_snapshot_metadata written_metadata;
  ASSERT_EQ(Preserve_snapshot_status::OK,
            sink.publish_bundle(std::move(bundle), 300, &written_metadata));
  EXPECT_GT(written_metadata.created_at_us, 0U);
  EXPECT_GT(written_metadata.expires_at_us, written_metadata.created_at_us);
  EXPECT_TRUE(receiver_metadata.token.empty());
  EXPECT_GT(frame_sink.frame_count(), 0U);
...
TEST_F(PreserveSnapshotTest,
  ...
  const uint64_t transfer_token = 603;
  const std::string record_payload =
      record_locks_payload() + "transfer-prebuilt-record-locks";
  const std::string binlog_payload = "transfer-prebuilt-binlog-cache";

  Local_file_preserved_trx_carrier warm_carrier(m_dir);
  PrebuiltRecordLocksBlob prebuilt_record;
  create_warm_prebuilt_record_locks_blob(
      &warm_carrier, "transfer-record-warmcopy", record_payload,
      &prebuilt_record);
  PrebuiltBinlogCacheBlob prebuilt_binlog;
  create_warm_prebuilt_binlog_blob(&warm_carrier, "transfer-binlog-warmcopy",
                                   binlog_payload, &prebuilt_binlog);
...
TEST_F(PreserveSnapshotTest, ArtifactSinkFactoryUsesSourceEpochSession) {
  ...
  Transfer_receiver_payload_test_sink frame_sink(m_dir, &store, &registry,
                                                 &receiver_metadata);
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-factory-session", "source-uuid", "target-uuid", 17,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            preserve_trx_make_artifact_sink_for_decision(
                Preserve_trx_transfer_artifact_decision::STANDBY_TRANSFER_SAVE,
                &store, "epoch-factory-session", "source-uuid", "target-uuid",
                transfer_token, 17, nullptr, &sink, &session, m_dir));
  ASSERT_NE(nullptr, sink.get());
  ASSERT_EQ(Preserve_snapshot_status::OK,
            sink->publish_bundle(std::move(bundle), 300, nullptr));
  EXPECT_FALSE(preserve_trx_transfer_epoch_committed(
...
TEST_F(PreserveSnapshotTest,
  ...
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-factory-session-queued", "source-uuid", "target-uuid", 17,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(1U, frame_sink.frames().size());
  ...
  ASSERT_EQ(Preserve_snapshot_status::OK,
            preserve_trx_make_artifact_sink_for_decision(
                Preserve_trx_transfer_artifact_decision::STANDBY_TRANSFER_SAVE,
                &store, "epoch-factory-session-queued", "source-uuid",
                "target-uuid", transfer_token, 17, nullptr, &sink, &session,
                m_dir));
  ASSERT_NE(nullptr, sink.get());
  ...
  EXPECT_EQ(1U, frame_sink.frames().size())
      << "phase2 per-token publish must queue final metadata instead of "
         "sending one command per token";

  ASSERT_EQ(Preserve_trx_transfer_status::OK, session.commit_epoch());
  ASSERT_EQ(2U, frame_sink.frames().size());

  Preserved_trx_store receiver_store(&carrier);
  Preserve_trx_transfer_receiver_registry registry;
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            preserve_trx_transfer_handle_receiver_payload_batch(
...
TEST_F(PreserveSnapshotTest,
  ...
  const uint64_t transfer_token = 704;
  const std::string record_payload =
      record_locks_payload() + "phase1-prewarm-manifest-record-locks";
  Mysql_binlog_preserve_snapshot binlog_snapshot;
  binlog_snapshot.cache_payload = "phase1-prewarm-manifest-binlog-cache";
  binlog_snapshot.gtid_next = "AUTOMATIC";
  binlog_snapshot.event_counter = 24;
  binlog_snapshot.with_rbr = true;
  binlog_snapshot.with_start = true;
  binlog_snapshot.with_content = true;
...
TEST_F(PreserveSnapshotTest,
       TransferSourceEpochSessionRejectsTokenAbortWithQueuedFinalMetadata) {
  Transfer_codec_context_guard codec_guard;
  const uint64_t transfer_token = 706;
  Capturing_transfer_frame_sink frame_sink;
  Preserve_trx_transfer_source_epoch_session session(
      "epoch-queued-final-abort", "source-uuid", "target-uuid", 17,
      &frame_sink);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.declare_token(transfer_token));
  ASSERT_EQ(1U, frame_sink.frames().size());
  ...
  descriptor.total_size = 4;
  descriptor.digest = test_sha256("snap");
  manifest.objects.push_back(descriptor);
  ASSERT_EQ(Preserve_trx_transfer_status::OK,
            session.begin_token_objects(manifest, true));
  ASSERT_EQ(1U, frame_sink.frames().size());

  EXPECT_EQ(Preserve_trx_transfer_status::UNSUPPORTED,
            session.abort_token(transfer_token,
                                "source_session_failure_after_queue"));
  EXPECT_EQ(1U, frame_sink.frames().size())
...
TEST_F(PreserveSnapshotTest, RecordLocksPagePlanCountsDistinctPages) {
  trx_preserve_record_lock_page_plan_t page_plan;

  ASSERT_TRUE(trx_preserve_record_lock_payload_page_plan(
      record_locks_payload_two_bits_one_entry(), &page_plan));
  EXPECT_EQ(1, page_plan.page_count);
  EXPECT_EQ(1, page_plan.bitmap_pages);
  EXPECT_EQ(2, page_plan.bitmap_bits);
}

TEST_F(PreserveSnapshotTest, RecordLocksPagePlanDeduplicatesPageIds) {
  std::string payload = record_locks_payload_two_bits_one_entry();
  payload[0] = 2;
  payload.append(payload.data() + 4, payload.size() - 4);

  trx_preserve_record_lock_page_plan_t page_plan;
  ASSERT_TRUE(trx_preserve_record_lock_payload_page_plan(payload, &page_plan));
  EXPECT_EQ(1, page_plan.page_count);
  EXPECT_EQ(2, page_plan.bitmap_pages);
  EXPECT_EQ(4, page_plan.bitmap_bits);
}

TEST_F(PreserveSnapshotTest, RecordLocksPagePlanSkipsPredicateEntries) {
  trx_preserve_record_lock_page_plan_t page_plan;

  ASSERT_TRUE(trx_preserve_record_lock_payload_page_plan(
      predicate_record_locks_payload(9), &page_plan));
  EXPECT_EQ(0, page_plan.page_count);
  EXPECT_EQ(0, page_plan.bitmap_pages);
  EXPECT_EQ(0, page_plan.bitmap_bits);
}

TEST_F(PreserveSnapshotTest, RecordLocksPagePlanRejectsMalformedPayload) {
  std::string malformed;
  append_le32(&malformed, UINT32_MAX);

  trx_preserve_record_lock_page_plan_t page_plan;
  EXPECT_FALSE(trx_preserve_record_lock_payload_page_plan(malformed,
                                                          &page_plan));
}

TEST_F(PreserveSnapshotTest, RecordLockPageResidencyEmptyPayloadIsReady) {
  trx_preserve_record_lock_residency_t residency;
  ASSERT_TRUE(trx_preserve_record_lock_payload_residency("", &residency));
  EXPECT_EQ(0U, residency.page_count);
  EXPECT_EQ(0U, residency.resident_pages);
  EXPECT_EQ(0U, residency.io_pending_pages);
  EXPECT_EQ(0U, residency.missing_pages);
}

TEST_F(PreserveSnapshotTest, RecordLockPageResidencyCountsPayloadPages) {
  trx_preserve_record_lock_residency_t residency;
  ASSERT_TRUE(trx_preserve_record_lock_payload_residency(record_locks_payload(),
                                                        &residency));
  EXPECT_EQ(1U, residency.page_count);
  EXPECT_EQ(0U, residency.resident_pages);
  EXPECT_EQ(0U, residency.io_pending_pages);
  EXPECT_EQ(1U, residency.missing_pages);
}

TEST_F(PreserveSnapshotTest, RecordLockPageResidencyRejectsMalformedPayload) {
  std::string malformed;
  append_le32(&malformed, UINT32_MAX);

  trx_preserve_record_lock_residency_t residency;
  EXPECT_FALSE(
      trx_preserve_record_lock_payload_residency(malformed, &residency));
}
