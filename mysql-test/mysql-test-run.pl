  ...
  foreach my $mysqld (mysqlds()) {
    ...
    if (-f $expect_file) {
      ...
      for (my $waits = 0 ; $waits < 50 ; mtr_milli_sleep(100), $waits++) {
        ...
        # If last line begins "restart:" or "restart_abort:", the rest of the
        # line is read as extra command line options to add to the restarted
        # mysqld. The restart_abort form leaves the expect file in "wait" so
        # tests can wait for the restarted server to crash during startup.
        # Anything other than 'wait', 'restart:' or 'restart_abort:' (with a
        # colon) will result in a restart with original mysqld options.
        my $follow_up_wait = 0;
