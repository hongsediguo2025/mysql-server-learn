...
| R1 / F1b terminal leg | debug-contract two-mysqld | `t/xfer2_staging_load_corrupt_partial_selection_w04x3.{test,result}` | `t/xfer2_staging_load_corrupt_partial_selection_w04x3.cnf` | Three independent W04 transactions over 32 accounts reach one real STANDBY_TRANSFER_SAVE epoch with receiver prewarm paused until terminal admission; a one-shot debug fault makes exactly one token's staging load return CORRUPT | the corrupted token becomes an exact `TOKEN_CORRUPT_ARTIFACT` partial-ready partition failure, two siblings stay READY and are saved/prepared, selection publishes (epoch global failure would have blocked it), and inflight/staged/debt state returns to baseline; both restarts prove source rollback and receiver emptiness | passing; RED on old code: classification never publishes after the epoch-level escalation |
| R1 / F1b transient leg | debug-contract two-mysqld | `t/xfer2_staging_load_io_error_deadline_partition_w04x3.{test,result}` | `t/xfer2_staging_load_io_error_deadline_partition_w04x3.cnf` | Three independent W04 transactions over 32 accounts; a persistent debug fault makes every staging load return IO_ERROR while receiver prewarm runs after terminal admission | each token retries inside its bounded budget with no terminal classification, failure record, debt, or epoch poison while transiently failing, then all three classify as exact `PREWARM_DEADLINE` when the reaper classifies at the deadline; on old code the first IO_ERROR escalated to epoch global failure and no classification ever published | passing; RED on old code: retry-budget wait never satisfied |
| H1 / R1 family, F1a face 1 | debug-contract two-mysqld | `t/xfer2_abort_deferred_while_prewarm_active_w04.{test,result}` | `t/xfer2_abort_deferred_while_prewarm_active_w04.cnf` | One W04 transaction; a receiver object-prewarm worker parks at `xfer2_prewarm_object_read_gate` (after the first terminal check, before any lookup/read); the source DRAIN parks at the phase-1 boundary; RESET DRAIN wins and sends one authenticated token ABORT | the ABORT publishes ABORTED keeping the in-memory wire objects and defers destructive cleanup to exactly one cleanup debt while the worker is parked (`abort_cleanup_deferred:active_prewarm`); after release the worker finishes without reading deleted files and the debt reaper completes the deletion; the owner is restored and rolls back | passing; RED on old code: no debt is ever registered because the ABORT deletes staging under the active worker |
| H1 / R1 family, F1a face 3 | debug-contract two-mysqld | `t/xfer2_commit_apply_failure_defers_cleanup_w04x2.{test,result}` | `t/xfer2_commit_apply_failure_defers_cleanup_w04x2.cnf` | Two W04 transactions; a receiver object-prewarm worker parks at `xfer2_prewarm_object_read_gate`; the owning COMMIT apply parks at `xfer2_commit_gate_a` and is failed once by a debug fault | the COMMIT failure publishes CORRUPT keeping in-memory objects, defers staging deletion to exactly one cleanup debt, and skips the epoch-wide derived-state purge while the worker is active; the source settles through the commit-anchored ABANDON, stays undecided on the degraded answer, and hands off fail-closed as `COMMIT_UNKNOWN` with both tokens quarantined | passing; RED on old code: no debt is ever registered |
| R3 / F2 | debug-contract two-mysqld | `t/xfer2_commit_partial_failure_abandon_settles_w04x2.{test,result}` | `t/xfer2_commit_partial_failure_abandon_settles_w04x2.cnf` | Two W04 transactions; a one-shot debug fault makes the last final-metadata payload never reach the wire with a deterministic (non-uncertain) failure, so COMMIT is never sent | the source settles the epoch through the commit-anchored ABANDON (no data-sequence reuse): receiver answers NOT_COMMITTED_CLEAN with one terminal CAS and one bounded tombstone, every token staging/inflight budget returns to baseline with no cleanup debt, and the source restores both owners locally without entering COMMIT_UNKNOWN; on old code the rewound per-token ABORTs collide with admitted sequences and leave tokens RECEIVING | passing; RED on old code: DRAIN fails with the wrong error contract and no abandon tombstone appears |
