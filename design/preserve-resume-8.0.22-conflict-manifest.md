...
This manifest records the known conflict and risk surface for the
preserve/resume backport from the 8.0.45-based feature stack to MySQL 8.0.22.
It was refreshed after the 8.0.22 port full-gate run so later review does not
reuse the stale early 30/66 counts.
...
git merge-tree --messages --name-only --merge-base=8.0 \
  mysql-8.0.22 7caefcc30efd79df3c5e2a38eb2b98a94461bd0f |
  awk 'NR > 1 && NF == 1 { print } /^$/ { exit }'
```
...
The first command returns status 1 because conflicts are expected and produces
34 explicit conflict files. The second command produces 70 changed-both files
and must be reviewed before final acceptance.
...
| Handler / memory helpers | `sql/handler.cc`, `sql/memory/aligned_atomic.h` |
...
| XA layout | `sql/xa/sql_xa_prepare.cc`, `sql/xa/sql_xa_start.cc` |
...
| InnoDB temp / trx runtime | `storage/innobase/srv/srv0tmp.cc`, `storage/innobase/trx/trx0purge.cc`, `storage/innobase/trx/trx0roll.cc`, `storage/innobase/trx/trx0trx.cc`, `storage/innobase/trx/trx0undo.cc` |
...
Every batch review must check both the 34 explicit conflict files and the
70-file changed-both overlap for the files touched by that batch.
