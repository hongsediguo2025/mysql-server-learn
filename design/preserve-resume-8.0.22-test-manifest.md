...
| `mysql-test/suite/preserve_trx/t/batch_drain_syntax_feature_gate.test` | `mysql-test/suite/preserve_trx/r/batch_drain_syntax_feature_gate.result` | Batch 0 | ported | 8.0.22 staging parser/feature-gate shell for DRAIN. RED failed on `DRAIN ... WITH TIMEOUT 300 NO USER VARS` syntax; GREEN passed debug/release normal and `--skip-log-bin` on 2026-06-16. `sql/preserve_trx_drain.{cc,h}` is now build-imported, but full batch-drain runtime remains Batch 4. |
...
| `mysql-test/suite/preserve_trx/t/detach_claim_rollback_staging_debug.test` | `mysql-test/suite/preserve_trx/r/detach_claim_rollback_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for the first real detached prepared lifecycle primitive. RED first hit 8.0.22 rollback-state assertions that accepted only ACTIVE/PREPARED; GREEN passed debug single-test MTR on 2026-06-16 after allowing PRESERVED rollback graph creation and lock release, detaching a real magic prepared transaction, claiming it, rolling it back, and proving the row update was undone. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; durable snapshot creation and RESUME attach remain pending. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/isolation_restore_staging_debug.test` | `mysql-test/suite/preserve_trx/r/isolation_restore_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for isolation-level restore helper validation. RED produced no observable row; GREEN passed debug single-test MTR on 2026-06-16 after porting valid SQL isolation mapping and invalid-byte rejection. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; positive RESUME-time isolation restore remains pending. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/kernel_preflight_introspection_staging_debug.test` | `mysql-test/suite/preserve_trx/r/kernel_preflight_introspection_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for read-only InnoDB current-THD preflight helpers. RED produced no observable kernel row; GREEN passed debug single-test MTR on 2026-06-16 after porting ReadView detection, no-redo undo state inspection, autoinc lock inspection, and modified-table name export. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/read_view_payload_export_staging_debug.test` | `mysql-test/suite/preserve_trx/r/read_view_payload_export_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for ReadView payload export/parse/validation helpers. RED produced no observable payload row; GREEN passed debug single-test MTR on 2026-06-16 after porting `Preserve_read_view_snapshot`, MVCC export/import helpers, and payload validity checks. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; positive resume-time ReadView import remains pending. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/read_view_import_staging_debug.test` | `mysql-test/suite/preserve_trx/r/read_view_import_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging guard for ReadView import. RED produced no observable row. A first runtime import attempt correctly exposed that `MVCC::preserve_import_view()` is recovery-only and asserts unless purge is `INIT` or `DISABLED`; GREEN now fails closed before closing the current RR view, proves the active snapshot remains stable, and records `import_ok=0` without crashing. Positive RESUME-time ReadView import remains pending for the recovery/attach slice. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/savepoint_import_staging_debug.test` | `mysql-test/suite/preserve_trx/r/savepoint_import_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for savepoint import helper validation. RED produced no observable row; a first GREEN attempt showed SQL names were wrong for InnoDB savepoint import in 8.0.22. GREEN passed debug single-test MTR on 2026-06-16 after deriving engine savepoint names from SQL savepoint handles and proving `ROLLBACK TO SAVEPOINT sp1` still works. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; full RESUME savepoint restore remains pending. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/savepoint_payload_export_staging_debug.test` | `mysql-test/suite/preserve_trx/r/savepoint_payload_export_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for savepoint payload export/validation helpers. RED produced no observable savepoint row; GREEN passed debug single-test MTR on 2026-06-16 after porting current-THD savepoint export and payload count validation. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; positive resume-time savepoint import remains pending. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/table_lock_payload_export_staging_debug.test` | `mysql-test/suite/preserve_trx/r/table_lock_payload_export_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for InnoDB table-lock payload export, parser, count, and AUTO_INC detection helpers. RED produced `export_ok=0`; GREEN passed debug single-test MTR on 2026-06-16 after exporting a real IX table lock from an UPDATE transaction and validating the payload. Public SQL still returns `ER_PRESERVE_TRX_UNSUPPORTED`; table-lock import and positive RESUME behavior remain pending. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/resume_acceptance_staging_debug.test` | `mysql-test/suite/preserve_trx/r/resume_acceptance_staging_debug.result` | Batch 2 | ported | 8.0.22 target-only debug staging test for idle-target THD acceptance before RESUME attach. RED returned not-found with no observable row; GREEN passed debug single-test MTR on 2026-06-16 after porting `trx_preserve_thd_can_accept_preserved_trx()`. Public RESUME still returns `ER_PRESERVE_TRX_UNSUPPORTED`; attach remains pending. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/resume_registry_lookup_staging_debug.test` | `mysql-test/suite/preserve_trx/r/resume_registry_lookup_staging_debug.result` | Batch 1 | ported | 8.0.22 target-only debug staging test for parser token preservation and registry-hit vs missing-token RESUME lookup split. RED returned not-found for an injected registry token; GREEN passed debug single-test MTR and four-mode migrated-shell regression on 2026-06-16. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/pfs_preserved_transactions_empty.test` | `mysql-test/suite/preserve_trx/r/pfs_preserved_transactions_empty.result` | Batch 1 | ported | 8.0.22 staging test. RED failed on missing `performance_schema.preserved_transactions`; GREEN passed debug/release normal and release `--skip-log-bin` on 2026-06-16. The table now scans through the shared empty `preserved_trx_snapshot()` view shell and passed the post-PFS-view migrated MTR regression in all four modes; registry-backed rows remain pending. |
| `mysql-test/suite/preserve_trx/t/pfs_preserved_transactions_acl_debug.test` | `mysql-test/suite/preserve_trx/r/pfs_preserved_transactions_acl_debug.result` | Batch 1 | ported | 8.0.22 target-only debug guard for registry-row ACL filtering and token redaction. RED showed unrelated accounts could see the row and RESUME_ANY saw the full token; GREEN passed debug single-test MTR and four-mode migrated-shell regression on 2026-06-16. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/token_delivery_finalize_staging_debug.test` | `mysql-test/suite/preserve_trx/r/token_delivery_finalize_staging_debug.result` | Batch 1 | ported | 8.0.22 target-only debug staging test for token-delivery finalization after the OK response. RED returned invalid-state before any post-response hook; GREEN passed debug single-test MTR and four-mode migrated-shell regression on 2026-06-16. Release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/token_delivery_release_resources_staging_debug.test` | `mysql-test/suite/preserve_trx/r/token_delivery_release_resources_staging_debug.result` | Batch 1 | ported | 8.0.22 target-only debug staging test for disconnect fallback finalization. RED showed `COM_QUIT` could overwrite the cached PREPARE OK response and remove the token; GREEN passed debug single-test MTR on 2026-06-16 after first-response-only caching. Release skip is expected through `have_debug.inc`. |
...
| `mysql-test/suite/preserve_trx/t/unsupported_cases.test` | `mysql-test/suite/preserve_trx/r/unsupported_cases.result` | Batch 0 | ported | 8.0.22 staging unsupported-context coverage for XA, non-InnoDB writes, temp tables, user locks, HANDLER, backup locks, LOCK TABLES, stored routine context, no preserved rows, and clean preserve dir. It now exercises the temp-table unsupported-context shell through helper-based admission rather than a direct `THD::temporary_tables` guard. Passed debug/release `--skip-log-bin` on 2026-06-16; normal-binlog runs skip. |
...
| `mysql-test/suite/preserve_trx/r/batch_drain_syntax_feature_gate.result` | Batch 0 | ported | Paired with ported `batch_drain_syntax_feature_gate.test`; parser/feature-gate shell only. DRAIN orchestrator source is build-imported but not called by runtime yet. |
...
| `mysql-test/suite/preserve_trx/r/detach_claim_rollback_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `detach_claim_rollback_staging_debug.test`; prepare/detach/claim/rollback primitive staging only. |
...
| `mysql-test/suite/preserve_trx/r/isolation_restore_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `isolation_restore_staging_debug.test`; isolation-level restore helper staging only. |
| `mysql-test/suite/preserve_trx/r/kernel_preflight_introspection_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `kernel_preflight_introspection_staging_debug.test`; current-THD ReadView/no-redo undo/autoinc/modified-table preflight helper staging only. |
| `mysql-test/suite/preserve_trx/r/read_view_payload_export_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `read_view_payload_export_staging_debug.test`; ReadView payload export/import helper staging only. |
| `mysql-test/suite/preserve_trx/r/savepoint_import_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `savepoint_import_staging_debug.test`; savepoint import helper staging only. |
| `mysql-test/suite/preserve_trx/r/savepoint_payload_export_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `savepoint_payload_export_staging_debug.test`; savepoint payload export/validation helper staging only. |
| `mysql-test/suite/preserve_trx/r/table_lock_payload_export_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `table_lock_payload_export_staging_debug.test`; table-lock payload export/parser/count staging only. |
...
| `mysql-test/suite/preserve_trx/r/resume_acceptance_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `resume_acceptance_staging_debug.test`; idle-target THD acceptance helper staging only. |
...
| `mysql-test/suite/preserve_trx/r/resume_registry_lookup_staging_debug.result` | Batch 1 | ported | Paired with target-only debug `resume_registry_lookup_staging_debug.test`; registry lookup staging only. |
...
| `mysql-test/suite/preserve_trx/r/token_delivery_finalize_staging_debug.result` | Batch 1 | ported | Paired with target-only debug `token_delivery_finalize_staging_debug.test`; staging coverage for post-OK token finalization only. |
| `mysql-test/suite/preserve_trx/r/token_delivery_release_resources_staging_debug.result` | Batch 1 | ported | Paired with target-only debug `token_delivery_release_resources_staging_debug.test`; staging coverage for release-resources fallback finalization only. |
...
| `mysql-test/suite/preserve_trx/r/unsupported_cases.result` | Batch 0 | ported | Paired with ported `unsupported_cases.test`; staging unsupported-context shell only. |
...
|---|---|---|---|
| `unittest/gunit/innodb/trx0preserve-t.cc` | Batch 2 | pending | `trx0preserve-t or the 8.0.22 equivalent gunit binary`; the 2026-06-16 target slice build-imported `trx0preserve.h/.cc` as a fail-closed API shell, but this checkout still has no configured InnoDB preserve gunit target and no Batch 2 behavior claim. |
...
| `mysql-test/suite/preserve_trx/t/pfs_preserved_transactions_acl_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for P_S/SHOW account visibility and token redaction around the in-memory registry shell. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/pfs_preserved_transactions_observable_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for shared P_S/SHOW row rendering and the first in-memory registry shell. RED showed the shared provider returned no rows, the first GREEN attempt exposed wrong SHOW metadata for BIGINT columns, and the follow-up RED showed injected rows were not retained after clearing the debug flag. GREEN passed debug single-test MTR and four-mode migrated-shell regression on 2026-06-16; release skip is expected through `have_debug.inc`. |
| `mysql-test/suite/preserve_trx/t/resume_acceptance_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for RESUME target-THD acceptance helper. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/resume_registry_lookup_staging_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for RESUME token parser/`Sql_cmd` plumbing and registry-hit vs missing-token staging behavior. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/token_delivery_finalize_staging_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for the post-response token-delivery finalizer and `THD::release_resources()` fallback hook placement. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/token_delivery_release_resources_staging_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for `THD::release_resources()` fallback finalization when dispatch finalization is skipped. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/isolation_restore_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for isolation-level restore helper validation. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/kernel_preflight_introspection_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for ReadView/no-redo undo/autoinc/modified-table preflight helpers. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/read_view_payload_export_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for ReadView payload export/parse/validation helpers. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/savepoint_import_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for savepoint import helper validation and SQL savepoint handle compatibility. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/savepoint_payload_export_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for savepoint payload export/validation helpers. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/table_lock_payload_export_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for table-lock payload export/parser/count helpers. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
| `mysql-test/suite/preserve_trx/t/detach_claim_rollback_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for the first real prepare/detach/claim/rollback lifecycle primitive. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
