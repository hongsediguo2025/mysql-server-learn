...
| `mysql-test/suite/preserve_trx/r/kernel_preflight_introspection_staging_debug.result` | Batch 2 | ported | Paired with target-only debug `kernel_preflight_introspection_staging_debug.test`; current-THD preflight helper staging only. |
...
| `mysql-test/suite/preserve_trx/t/kernel_preflight_introspection_staging_debug.test` | Batch 2 | ported | 8.0.22 target-only debug guard for ReadView/no-redo undo/modified-table preflight helpers. RED/GREEN evidence is recorded in the paired MTR test-file row above. |
