...
| `mysql-test/suite/preserve_trx/t/pfs_preserved_transactions_observable_debug.test` | Batch 1 | ported | 8.0.22 target-only debug guard for shared P_S/SHOW row rendering. RED showed the shared provider returned no rows, and the first GREEN attempt exposed wrong SHOW metadata for BIGINT columns. GREEN passed debug single-test MTR and four-mode migrated-shell regression on 2026-06-16; release skip is expected through `have_debug.inc`. |
